...
class project_savecomment
{
	public function index(){
		...
		$comment['deal_id'] = intval(base64_decode($GLOBALS["request"]['id']));
		...
		$comment['content'] = strim(base64_decode($GLOBALS["request"]['content']));
		$comment['user_id'] = $user_id;
		$comment['create_time'] =  TIME_UTC;
		$comment['user_name'] = $user['user_name'];
		$comment['pid'] = intval(base64_decode($GLOBALS["request"]['pid']));
		...
		if(intval(base64_decode($GLOBALS["request"]['syn_weibo']))==1)
		{
			$weibo_info = array();
