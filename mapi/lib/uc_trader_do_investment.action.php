...
class uc_trader_do_investment
{
	public function index(){
		...
		if ($user_id >0){
			...
			$bid_paypassword = strim(base64_decode($GLOBALS['request']['bid_paypassword']));
...
//			output($root);
			...
			if($GLOBALS['db']->affected_rows())
			{
				...
				modify_account(array('money'=>-$order_info['borrow_money'],'lock_money'=>$order_info['borrow_money']), $user_id,'配资出借冻结,配资编号:'.$order_info["id"],36);
