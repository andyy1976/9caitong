...
class register_idno{
	public function index()
	{
		...
		if ($user_id >0){
			...
			$real_name = strim(base64_decode($GLOBALS['request']['real_name']));
			$idno = strim(base64_decode($GLOBALS['request']['idno']));
			...
			if(intval(app_conf("OPEN_IPS")) > 0){
				$app_url = APP_ROOT."/index.php?ctl=collocation&act=CreateNewAcct&user_type=0&is_pc=1&user_id=".$user_id."&from=".base64_decode($GLOBALS['request']['from']);
