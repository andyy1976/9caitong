...
class project_save_item
{
	public function index(){
		...
		$data['price'] = floatval(base64_decode($GLOBALS["request"]['price']));
		if($data['price']<=0)
		{
			$root["status"] = 0;
		...
		$data['description'] = strim(base64_decode($GLOBALS["request"]['description']));
		$data['is_delivery'] = intval(base64_decode($GLOBALS["request"]['is_delivery']));
		$data['delivery_fee'] = floatval(base64_decode($GLOBALS["request"]['delivery_fee']));
		$data['is_limit_user'] = intval(base64_decode($GLOBALS["request"]['is_limit_user']));
		$data['limit_user'] = intval(base64_decode($GLOBALS["request"]['limit_user']));
		$data['repaid_day'] = intval(base64_decode($GLOBALS["request"]['repaid_day']));
		$data['deal_id'] = intval(base64_decode($GLOBALS["request"]['deal_id']));
		$data['is_share'] = intval(base64_decode($GLOBALS["request"]['is_share']));
		$data['share_fee'] = floatval(base64_decode($GLOBALS["request"]['share_fee']));
		...
		if(count(base64_decode($GLOBALS["request"]['file']))>4)
		{
			$root["status"] = 0;
		...
		if($id==0)
		{
			...
			if($result_id>0)
			{
				if(count(base64_decode($GLOBALS["request"]['file']))>=0)
				{
					foreach(base64_decode($GLOBALS["request"]['file']) as $k=>$v)
					{
		...
		else
		{
			...
			//旧图
			$old_images = '0';
			if(count(base64_decode($GLOBALS['request']['image_names']))>0)
			{
				foreach(base64_decode($GLOBALS['request']['image_names']) as $i_k => $i_v)
				{
