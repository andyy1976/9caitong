...
class project_cart_go_pay
{
	public function index(){
		...
		$consignee_id = intval(base64_decode($GLOBALS["request"]['consignee_id']));
		$ecv_id = intval(base64_decode($GLOBALS["request"]['ecv_id']));
		$pay_score =intval(base64_decode($GLOBALS["request"]['pay_score']));
		...
		$memo = strim(base64_decode($GLOBALS["request"]['memo']));
		//@by slf
		...
		//无私奉献
		if($deal_item['type']==1){
			$pay_money=floatval(base64_decode($GLOBALS["request"]['pay_money']));
			if($pay_money<=0){
				$data['pay_status'] = 0;
