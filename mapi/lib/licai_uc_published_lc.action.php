...
class licai_uc_published_lc
{
	public function index(){
		...
		if ($user_id >0){
			...
			if(strim(base64_decode($GLOBALS['request']["deal_name"]))!="")
			{
				$condition .= " and lc.name like '%".strim(base64_decode($GLOBALS['request']["deal_name"]))."%'";
				$search["deal_name"] = strim(base64_decode($GLOBALS['request']["deal_name"]));
			}
			
			if(strim(base64_decode($GLOBALS['request']["licai_type"]))!=="" && intval(base64_decode($GLOBALS['request']["licai_type"]))!=-1)
			{
				$condition .= " and lc.type = ".intval(base64_decode($GLOBALS['request']["licai_type"]));
				$search["licai_type"] = intval(base64_decode($GLOBALS['request']["licai_type"]));
			}
			else
			{
				$search["licai_type"] = -1;
			...
			$buy_begin_time = strim(base64_decode($GLOBALS['request']['buy_begin_time']));
			$buy_end_time = strim(base64_decode($GLOBALS['request']['buy_end_time']));
			...
			if(strim($buy_begin_time)!="")
			{
				...
				$search["buy_begin_time"] = strim(base64_decode($GLOBALS['request']["buy_begin_time"]));
				
			}
			if(strim($buy_end_time) !="")
			{
				$condition .= " and lc.begin_buy_date <= '".  strim($buy_end_time)."'";
				$search["buy_end_time"] = strim(base64_decode($GLOBALS['request']["buy_end_time"]));
			}
			
			//项目结束时间
			$start_time = strim(base64_decode($GLOBALS['request']['begin_time']));
			$end_time = strim(base64_decode($GLOBALS['request']['end_time']));
