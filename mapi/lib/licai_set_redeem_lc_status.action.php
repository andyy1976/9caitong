...
class licai_set_redeem_lc_status
{
	public function index(){
		...
		if ($user_id >0){
			...
			$redempte_id = intval(base64_decode($GLOBALS['request']["redempte_id"]));
			$status = 1;
			$earn_money = strim(base64_decode($GLOBALS['request']["earn_money"]));
			$fee = strim(base64_decode($GLOBALS['request']["fee"]));
			$pay_type = 0; //0不允许垫付
			$web_type = 2; //0前台
