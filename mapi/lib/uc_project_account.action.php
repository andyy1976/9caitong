...
class uc_project_account
{
	public function index(){
		...
		$page = intval(base64_decode($GLOBALS["request"]['p']));
		if($page==0)
			$page = 1;
		$limit = (($page-1)*$page_size).",".$page_size;
		
  		$more_search=intval(base64_decode($GLOBALS["request"]['more_search']));
 		$root['more_search']=$more_search;
 		$parameter[]="more_search=".$more_search;
		
		$deal_name=strim(base64_decode($GLOBALS["request"]['deal_name']));
		...
		$give_status=intval(base64_decode($GLOBALS["request"]['give_status']));
		if($give_status>0){
			if($deal_status==3){
				switch($give_status){
					//已发放
		...
		$begin_time=strim(base64_decode($GLOBALS["request"]['begin_time']));
 		if($begin_time!=0){
