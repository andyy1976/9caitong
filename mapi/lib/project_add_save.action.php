...
class project_add_save
{
	public function index(){
		...
		$data['name'] = strim(base64_decode($GLOBALS["request"]['name']));
		if($data['name']=="")
		{
			$root["status"] = 0;
		...
		$data['cate_id'] = intval(base64_decode($GLOBALS["request"]['cate_id']));
		if($data['cate_id']==0)
		{
			$root["status"] = 0;
		...
		$data['province'] = strim(base64_decode($GLOBALS["request"]['province']));
		if($data['province']=='')
		{
			$root["status"] = 0;
			$root["show_err"] = "请选择省份";
		...
		$data['city'] = strim(base64_decode($GLOBALS["request"]['city']));
		if($data['city']=='')
		{
			$root["status"] = 0;
			$root["show_err"] = "请选择城市";
		...
		$data['brief'] = strim(base64_decode($GLOBALS["request"]['brief']));
		$data['image'] = replace_public(addslashes(trim(base64_decode($GLOBALS["request"]['image_file']))));
		if($data['image']=="" && $id == 0)
		{			
			$root["status"] = 0;
		...
		$data['description'] = replace_public(addslashes(trim(valid_tag(base64_decode($GLOBALS["request"]['description'])))));	
//		
		$data['vedio'] = strim(base64_decode($GLOBALS["request"]['vedio']));
		...
		$data['limit_price'] = floatval(base64_decode($GLOBALS["request"]['limit_price']));
		...
		$data['deal_days'] = floatval(base64_decode($GLOBALS["request"]['deal_days']));
		if($data['deal_days']<=0)
		{
			$root["status"] = 0;
		...
		if($id>0)
		{
			$savenext = intval(base64_decode($GLOBALS["request"]['savenext']));
			...
			$sort = 1;
			foreach(base64_decode($GLOBALS["request"]['question']) as $kk=>$question_item)
			{
				if(strim(base64_decode($GLOBALS["request"]['question'][$kk]))!=""&&strim(base64_decode($GLOBALS["request"]['answer'][$kk]))!=""&&strim(base64_decode($GLOBALS["request"]['question'][$kk]))!="请输入问题"&&strim(base64_decode($GLOBALS["request"]['answer'][$kk]))!="请输入答案")
				{
					$faq_item['deal_id'] = $id;
					$faq_item['question'] = strim(base64_decode($GLOBALS["request"]['question'][$kk]));
					$faq_item['answer'] = strim(base64_decode($GLOBALS["request"]['answer'][$kk]));
					$faq_item['sort'] = $sort;
