...
class doexchange
{
	public function index(){
		...
		$address_id = intval(base64_decode($GLOBALS['request']['address_id']));
		$goods_id = intval(base64_decode($GLOBALS['request']['goods_id']));
		$number = intval(base64_decode($GLOBALS['request']['number']));
		//检查用户,用户密码
		$root['user_info'] = $user;
