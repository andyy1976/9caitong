	...
	public function index(){
		...
		if ($user_id >0){
	    	...
	    	if($type == "credit_identificationscanning"){
	    		$u_c_data['real_name_encrypt'] = "AES_ENCRYPT('".strim(base64_decode($GLOBALS['request']['real_name']))."','".AES_DECRYPT_KEY."')";
	    		$u_c_data['idno_encrypt'] = "AES_ENCRYPT('".strim(base64_decode($GLOBALS['request']['idno']))."','".AES_DECRYPT_KEY."')";
                $u_c_data['byear'] = substr(base64_decode($GLOBALS['request']['idno']),6,4);
                $u_c_data['bmonth'] = substr(base64_decode($GLOBALS['request']['idno']),10,2);
                $u_c_data['bday'] = substr(base64_decode($GLOBALS['request']['idno']),12,2);
	    		if(getIDCardInfo(strim(base64_decode($GLOBALS['request']['idno'])))==0){
	    	...
	    	if($credit_type['list'][$type]['file_count'] > 0){
		    	for($i=0;$i<=$credit_type['list'][$type]['file_count']-1;$i++){
		    		if(base64_decode($GLOBALS['request']['is_wap'])==1){
		    			if(trim(base64_decode($GLOBALS['request']['file'][$i]))!=""){
		    				$file[] = base64_decode($GLOBALS['request']['file'][$i]);				
		    			}
	    			}
	    			else{
