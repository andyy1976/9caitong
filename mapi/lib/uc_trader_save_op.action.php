...
class uc_trader_save_op
{
	public function index(){
		...
		if ($user_id >0){
			...
			$peizi_order_id = intval(base64_decode($GLOBALS['request']["id"]));
			$op_type = intval(base64_decode($GLOBALS['request']["type"]));
			$op_val = strim(base64_decode($GLOBALS['request']["op_val"]));
