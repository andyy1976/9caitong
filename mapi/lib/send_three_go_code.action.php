...
class send_three_go_code{
    ...
    public function index(){
        ...
        if($user['id']>0){
            if(empty($real_name) || empty($idno) || empty($bank) || empty($bank_card) || empty($mobile)){
            ...
            $userbank = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_bank where cunguan_tag=1 and bankcard='".$bank_card."'");
            $usbank = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_bank where cunguan_tag = 1 user_id=".$user['id']." and status=1");
            if($userbank){
                $root['response_code'] = 0;
