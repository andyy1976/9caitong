...
class project_deals
{
	public function index(){
        ...
        $param = array();//参数集合
         ...
         //数据来源参数
		$r = strim(base64_decode($GLOBALS['request']['r']));   //推荐类型
        $param['r'] = $r?$r:'';
		$root['p_r'] = $r;
                
		$id = intval(base64_decode($GLOBALS['request']['id']));  //分类id
		$param['id'] = $id;
		$root['p_id'] = $id;
		
		$loc = strim(base64_decode($GLOBALS['request']['loc']));  //地区
		$param['loc'] = $loc;
		$root['p_loc'] = $loc;
        
        $state = intval(base64_decode($GLOBALS['request']['state']));  //状态

        $param['state'] = $state;
		$root['p_state'] = $state;
                
		$tag = strim(base64_decode($GLOBALS['request']['tag']));  //标签
		$param['tag'] = $tag;
		$root['p_tag'] = $tag;
                
		$kw = strim(base64_decode($GLOBALS['request']['k']));    //关键词
		$param['k'] = $kw;
		$root['p_k'] = $kw;
		        
        $type = intval(base64_decode($GLOBALS['request']['type']));   //推荐类型
        $param['type'] = $type;
		$root['p_type'] = $type; 

		if(intval(base64_decode($GLOBALS['request']['redirect']))==1)
		{
			$param = array();
			if($r!="")
			{
       ...
       if($type ==0){
		...
		$step = intval(base64_decode($GLOBALS['request']['step']));
		if($step==0)$step = 1;
