...
//			exit;
		...
		$conf_id = intval(base64_decode($GLOBALS['request']['conf_id']));
		$is_today = intval(base64_decode($GLOBALS['request']['is_today']));
		$borrow_money = intval(base64_decode($GLOBALS['request']['borrow_money']));//需要借款金额
		$lever = intval(base64_decode($GLOBALS['request']['lever']));//	
		$rate_id = intval(base64_decode($GLOBALS['request']['rate_id']));//
		...
		if ($type == 1){
			...
			$time_limit_num = intval(base64_decode($GLOBALS['request']['time_limit_num']));//资金使用期限			
		...
		if($user_total_money< $total_money){
			...
			$root["jump"] = WAP_SITE_DOMAIN.url("member","uc_money#incharge",array('money'=>$total_money));//member.php?ctl=uc_money&act=incharge
//			echo json_encode($root);
//			exit;
			output($root);	
		}
