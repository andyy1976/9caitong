...
//			$red = intval(base64_decode($GLOBALS['request']['red']));   // 老版红包   已弃用
            ...
            $red_money = $GLOBALS['db']->getOne("select sum(money)  from ".DB_PREFIX."red_packet where user_id=".$user_id." and id in(".$red_id.")" );//红包金额
            $bid_money = intval(base64_decode($GLOBALS['request']['bid_money']));    //本金
            $cunguan_tag=$GLOBALS['db']->getOne("select cunguan_tag from ".DB_PREFIX."deal where id=".$id);
//            if($cunguan_tag==0){
...
//            $map['bid_paypassword'] = $paypwd;
            ...
            if($map['interestrate_id']){
                $map['interestrate_money'] = get_interestrate_money($map['interestrate_id'],$map['bid_money'],$map['deal_id']);    //加息收益
            }
			$status = dobid_app($map);
			if($status['status'] == 1){
