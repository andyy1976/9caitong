...
class project_savecomment
{
	public function index(){
		...
		{
			$data['log_info'] = strim(base64_decode($GLOBALS["request"]['log_info']));
			if($data['log_info']=="")
			{
				$root['response_code'] = 0;
				$root['show_err'] = "请输入更新的内容";
			}
			$data['image'] = strim(base64_decode($GLOBALS["request"]['image']))!=""?replace_public(base64_decode($GLOBALS["request"]['image'])):"";
			$data['vedio'] = strim(base64_decode($GLOBALS["request"]['vedio']));
			if($data['vedio']!="")
			{
				require_once APP_ROOT_PATH."system/utils/vedio.php";
				$vedio = fetch_vedio_url(base64_decode($GLOBALS["request"]['vedio']));
				if($vedio!="")
				{
