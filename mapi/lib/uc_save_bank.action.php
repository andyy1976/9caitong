...
class uc_save_bank
{
	public function index(){
		...
		if ($user_id >0){
			...
			$data['bank_id'] = intval(base64_decode($GLOBALS['request']['bank_id']));
			...
			$data['region_lv1'] = intval(base64_decode($GLOBALS['request']['region_lv1']));
			$data['region_lv2'] = intval(base64_decode($GLOBALS['request']['region_lv2']));
			$data['region_lv3'] = intval(base64_decode($GLOBALS['request']['region_lv3']));
			$data['region_lv4'] = intval(base64_decode($GLOBALS['request']['region_lv4']));
			
			$data['bankzone'] = trim(base64_decode($GLOBALS['request']['bankzone']));
			if($root['response_code'] == 1 && $data['bankzone'] == ""){
				$root['response_code'] = 0;
				$root['show_err'] = "请输入开户行网点";
			}
			
			$data['bankcard'] = trim(base64_decode($GLOBALS['request']['bankcard']));
