<?php 

// | p2p系统 mapi 插件

// | Copyright (c) 2010 http://www.jiuchengjr.com All rights reserved.


if(!defined("MAGIC_QUOTES_GPC")){
    @set_magic_quotes_runtime (0);
    define('MAGIC_QUOTES_GPC',get_magic_quotes_gpc()?True:False);
}

//过滤请求
function mapi_filter_request(&$request)
{
    if(MAGIC_QUOTES_GPC)
    {
        foreach($request as $k=>$v)
        {
            if(is_array($v))
            {
                mapi_filter_request($request[$k]);
            }
            else
            {
                $request[$k] = stripslashes(trim($v));
            }
        }
    }

}
$_REQUEST = array_merge($_GET,$_POST);
mapi_filter_request($_REQUEST);
if (isset($_REQUEST['i_type']))
{
    $i_type = intval(base64_decode($_REQUEST['i_type']));
}

if ($i_type == 1)
{
    $request = $_REQUEST;
}
else
{
    if (isset($_REQUEST['requestData']))
    {
        if ($i_type == 2)
        {
            $request = json_decode(trim($_REQUEST['requestData']), 1);
        }else if($i_type == 4){
			
        	$dir = str_replace('mapi/index.php', '', str_replace('\\', '/', __FILE__));
        	
			require_once $dir.'/system/libs/crypt_aes.php';
			$aes = new CryptAES();
			$aes->set_key('jc5LMUQC43P2P');
			$aes->require_pkcs5();
			//echo FANWE_AES_KEY; exit;
			//XqcrhQzAmFVYFLM1w80gU5ehUS19vtrZcLR26z8HtVzPMj082BfaMIaAlxrEINTaXr2ANG69bDqjQu80MeGfYMux7AA1Fqx8ofIeMtFfcg1IP2OfwS+meIM+VB4HBusuqkCRkT+eKv5UD/s1HZ5y9NCMFosa7qm84BOWHJTVsXc=
			//print_r($_REQUEST['requestData']);exit;
			$decString = $aes->decrypt(trim($_REQUEST['requestData']));

			//print_r($decString);exit;
			$request = json_decode($decString, 1);
		}else
        {
            //$_REQUEST['requestData'] ="";
            $request = base64_decode((trim($_REQUEST['requestData'])));
            $request = json_decode($request, 1);
        }
    }else
    {
        $request = $_REQUEST;
    }
}



require '../system/common.php';
require '../app/Lib/common.php';
require './lib/functions.php';
require '../system/libs/user.php';
require '../system/utils/Verify.class.php';
if(!defined("APP_INDEX"))
	define("APP_INDEX","index");
	

if (!isset($request['from']) || empty($request['from'])){
	$request['from'] = base64_encode('app');
}


$sessid = $request['session_id'];

if (empty($sessid)){
	$sessid = es_session::id();
	$request['session_id'] = $sessid;
}

es_session::set_sessid($sessid);
$m_config = getMConfig();//初始化手机端配置
$version = getVersion();//初始化手机端更新配置
define('VERSION',1); //接口版本号,float 类型
define("CACHE_TIME",60); //动态数据缓存时间，300秒
if (intval($m_config['page_size']) == 0){
	define('PAGE_SIZE',20); //分页的常量
}else{
	define('PAGE_SIZE',intval($m_config['page_size'])); //分页的常量
}
//获取移动端设备信息 存入session
if($GLOBALS['request']['MachineInfo']){

    $machine_info=explode('|||',base64_decode($GLOBALS['request']['MachineInfo']));
    $phone_msg = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."preload_statistics_log where phone_msg='".$machine_info[8]."'");
    if($machine_info[7] && $machine_info[8] && !$phone_msg){
        $data['phone_msg'] = $machine_info[8];
        $data['source'] = $machine_info[7];
        $data['open_time'] = $machine_info[9];
        $data['addtime'] = time();
        $GLOBALS['db']->autoExecute(DB_PREFIX."preload_statistics_log", $data, 'INSERT');
    }
	es_session::set('MachineInfo',base64_decode($GLOBALS['request']['MachineInfo']));
	$GLOBALS['MachineInfo'] = es_session::get('MachineInfo');
	if($GLOBALS['MachineInfo'] != base64_decode($GLOBALS['request']['MachineInfo'])){
		es_session::delete('MachineInfo');
	}
}
$class = strtolower(strim(base64_decode($request['act'])));
$act2 = strtolower(strim(base64_decode($request['act_2'])))?strtolower(strim(base64_decode($request['act_2']))):"";
$city_id = intval($request['city_id']);
define('ACT',$class); //act常量
define('ACT_2',$act2);
$GLOBALS['user_info'] = es_session::get("user_info");

if(empty($GLOBALS['user_info']) && $class != 'login')
{
	$cookie_uname = strim(base64_decode($request['email']));//用户名或邮箱
	$cookie_upwd = strim(base64_decode($request['pwd']));//密码

	if($cookie_uname!=''&&$cookie_upwd!='')
	{
		$cookie_uname = strim($cookie_uname);
		if (strlen($cookie_upwd) != 32)
			$cookie_upwd = md5($cookie_upwd);
		$cookie_upwd = md5($cookie_upwd."_EASE_COOKIE");
		auto_do_login_user($cookie_uname,$cookie_upwd);
		$GLOBALS['user_info'] = es_session::get('user_info');

	}
}
else{
    $GLOBALS['user_info'] = get_user_info("*","id=".$GLOBALS['user_info']['id']);
    es_session::set('user_info',$GLOBALS['user_info']);
}

if(true) 
{
	$url = WAP_SITE_DOMAIN.APP_ROOT."/index.php?requestData=".$_REQUEST['requestData']."&r_type=2";
	$api_log = array();
	$api_log['api'] = $url;
	$api_log['act'] = $class;
	$GLOBALS['db']->autoExecute(DB_PREFIX."api_log", $api_log, 'INSERT');
}
/*if($GLOBALS['user_info']){
	$sess = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."session where user_id ='".$GLOBALS['user_info']['id']."'");
    if($sess['session_id'] != $sessid){
    	require_once APP_ROOT_PATH."system/libs/user.php";
		$result = loginout_user();
		$root['response_code'] = 0;
        $root['response_login_code'] = 0;
        $root['show_err'] = "当前账号于".$sess['session_data']."在其他设备上登录，请注意账户安全。您已被迫下线，是否重新登录？";
        output($root);
    }else{
    	 $root['response_login_code'] = 1;
    }
}else{
    $root['response_login_code'] = 1;
}*/

$root['response_login_code'] = 1;
//公共初始化
if(file_exists("./lib/".$class.".action.php"))
{	
	require_once "./lib/".$class.".action.php";			
	if(class_exists($class))
	{
		$obj = new $class;		
		if(method_exists($obj,"index"))
		{
			$obj->index();
		}
		else
		{
			header("Content-Type:text/html; charset=utf-8");
			exit("Hack attemp!");
		}
	}
	else
	{
		header("Content-Type:text/html; charset=utf-8");
		exit("Hack attemp!");
	}
}
else
{
	header("Content-Type:text/html; charset=utf-8");
	exit("Hack attemp!".ACT);
}

?>