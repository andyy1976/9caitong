...
//        }
        ...
        */
        ...
        foreach ($switch as $k=>$v){
            if($v['status']!=1){
            	...
            	if($deal['type_id']==14){
                    //抵押物资料
                    $deal['mortgage_brand']= $deal['house_info']?$deal['house_info']:'暂无';
        ...
        if($deal['debts']==1){
            ...
            //$deal['debts_repay_time']=  ((strtotime(date("Y-m-d",$last_repay_time))-strtotime(date("Y-m-d",$deal['start_time'])))/3600/24)+1;
			$remin = strtotime(date("Y-m-d",$last_repay_time))-strtotime(date("Y-m-d",time()));
			if($remin>0){
				$deal['debts_repay_time']= ((strtotime(date("Y-m-d",$last_repay_time))-strtotime(date("Y-m-d",time())))/3600/24)+1;
			}else{
				$deal['debts_repay_time']=0;
			}
        }
		...
		if($deal['type_id']==14){
            $GLOBALS['tmpl']->display("page/deal_house.html");
		}else{
    ...
    */
	...
	public function success(){ //投标成功
		...
		$id = intval($_REQUEST['id']);
		$plan_load_id = intval($_REQUEST['plan_load_id']);
		$deal = get_deal($id);
		...
		if($plan_load_id){
			$GLOBALS['tmpl']->assign('plan_load_id',$plan_load_id);
		}
	...
	public function addmessage(){
		...
		function bid(){
			if (!empty($_REQUEST['deal_id'])) {
				//如果deal_id不为空视为普通标的
				$bidinfo[0]['deal_id'] = $_REQUEST['deal_id'];
				$bidinfo[0]['bid_money'] = $_REQUEST['bid_money']; //出借人投资金额
				$bidinfo[0]['red_id'] = $_REQUEST['red_id'];
				$bidinfo[0]['interestrate_id'] = $_REQUEST['interestrate_id'];

				//print_r($bidinfo[0]['bid_money']);die;
				foreach ($bidinfo as $key => $value) {
					$map = $this->bid_action($value);
				}
				$return["status"] = 1;
				...
				//接受理财计划参数
				$map['plan_id'] = $_REQUEST['plan_id'];  //接受理财计划的ID,是标的的id
				$map['bid_money'] = $_REQUEST['bid_money'];
				$map['red_id'] = $_REQUEST['red_id'];
				$map["interestrate_id"] = $_REQUEST['interestrate_id'];
				//生成理财计划数据
				$BidListSerial = $GLOBALS['db']->getRow("select deal_id, load_money, borrow_amount,rate,min_loan_money from " . DB_PREFIX . "plan  where id= ".$map["plan_id"]); //理财计划所包含标的
				$map['rate'] = $BidListSerial['rate'];
				$map['min_loan_money'] = $BidListSerial['min_loan_money'];
				
				//print_r($map['rate']);die;
            	...
            	if ($map["bid_money"] > $cunguan_money) {
	                $return["status"] = 4;
	            ...
	            $map['total_money']=$map["bid_money"]+$map['red_money'];  //用户红包加上投资金额等于可投金额
	            //保护 1、计划是否可投；
	            $plan_need_money = $BidListSerial['borrow_amount'] - $BidListSerial['load_money'];
	            if($plan_need_money >= $map['min_loan_money']){
	            	if(!$map["total_money"]){
	            		$return["status"] = 3;
					    $return["info"] = "起投金额为".$map['min_loan_money']."元";
					    ajax_return($return);
	            	}
	            	if ($map["total_money"] < $map['min_loan_money']) {
					    $return["status"] = 0;
					    $return["info"] = "您的投资额小于起投金额";
					    ajax_return($return);
					}
	            }else{
	            	if($map["total_money"]<$BidListSerial['min_loan_money']&&$map['total_money']!=$plan_need_money){
	            		$return["status"] = 0;
					    $return["info"] = "尾标需一次性投清";
					    ajax_return($return);
	            	}
	            }
	            
				if($plan_need_money < $map['total_money']){
					$return["status"] = 0;
	            ...
	            $GLOBALS['tmpl']->assign("bid_money",$map['bid_money']);
				...
				// if($map['interestrate_id']){
				//     $map['interestrate_money'] = get_interestrate_money($map['interestrate_id'],$deal_total_money,$deal_id,$plan_id); 
				// }
				$publics = new Publics();
				if($map['red_money']>0){
				...
				// print_r($deal_load_data);die;
				$GLOBALS['tmpl']->assign('plan_id',$map['plan_id']);
				$return["status"] = 1;
				$return["info"] = $GLOBALS['tmpl']->fetch("page/deal_bid.html");
				$return["pc_con"] = json_encode($deal_load_data);
				$return["jump"] = url("index","deal#deals");
				$return["planjump"] = url("index","plandeal#deals");
				ajax_return($return);
			}
			
		}
		function bid_action($bidinfo=array()){
			/*status状态码
			* 0 提示错误信息，无任何操作
			* 1 投标成功
			* 2 未登录，跳转登录页面
			...
			*****/
			...
			$deal_id = intval($bidinfo['deal_id']);
			$map['deal_id'] = $deal_id;
//			$map['bidmoney'] = intval(strim($_REQUEST['bid_money']));
			$map['bidmoney'] = strim($bidinfo['bid_money']);
            $map['red_id'] = strim($bidinfo['red_id']);
            //  红包金额
            if ($map['red_id']) {
            	$map['red_money'] =  $GLOBALS['db']->getOne("select sum(money) from ".DB_PREFIX."red_packet where id in(".$map['red_id'].")");
            }
            ...
            //  加息券
            $map['interestrate_id'] = strim($bidinfo['interestrate_id']);
            if($map['interestrate_id']){
...
//			$GLOBALS['tmpl']->assign("interestrate_money",$map['interestrate_money']);//代金券金额
	...
	function plan_dobid2(){
	//获取理财计划ID
			// $seqno = $load_seqnoa; //测试开启
			$seqno=$_GET['businessSeqNo']; //验密开启
			if(!$seqno){
				showErr('验密失败',0,url("index","deals"));
			}
			if(!$GLOBALS['user_info']['is_entrust']){//如果没有签署免密协议
				$reg = new Register();
				$res = $reg ->delegate($seqno,$GLOBALS['user_info']['id'],"B04","T01",$seqno);
				if($res['respHeader']['respCode'] == "P2P0000"){
                        //更改user表委托协议状态
                        $GLOBALS['db']->query("UPDATE ".DB_PREFIX."user SET is_entrust=1,entrustNo="."'$seqno'"." where id=".$GLOBALS['user_info']['id']);
                        
                 }else{
						if(WAP==1){
							$GLOBALS['tmpl']->assign('msg',"系统繁忙，请稍后再试");
							$GLOBALS['tmpl']->display("page/deal_fail.html");die;
						}else{
							$jump = url("index","plandeal");
			...
			// 取出临时表中的用户提交信息
			$plan_temp = $GLOBALS['db']->getRow("select plan_id,money,red,red_id,interestrate_id,total_money from " . DB_PREFIX . "deal_load_temp  where load_seqno= '".$seqno."'");
			$GLOBALS['db']->startTrans();   //开始事务
			$plan = $GLOBALS['db']->getRow("select deal_id,load_money,rate,repay_time,borrow_amount,name from ".DB_PREFIX."plan  where id=".$plan_temp["plan_id"]." FOR UPDATE"); //理财计划所包含标的
			if($plan_temp['total_money']+$plan['load_money']>$plan['borrow_amount']){
				$GLOBALS['db']->rollback();
				if(WAP==1){
					/* $return["status"] = 1;
					$return["info"] = "投资金额大于可投金额";
					ajax_return($return); */
					$jump = machineInfo();
					$GLOBALS['tmpl']->assign('jump',$jump);
					$GLOBALS['tmpl']->assign('msg',"投资金额大于可投金额");
					$GLOBALS['tmpl']->display("page/deal_fail.html");die;
				}else{
					showErr("投资金额大于可投金额",0,url("index","plandeal"));
				}
			}
			$load['rate'] = $plan['rate'];
			$load['plan_id'] = intval($plan_temp['plan_id']); //理财计划ID
			$load['money'] = intval(strim($plan_temp['money'])); //理财计划资金
			$load['ecv_id'] = $plan_temp['red_id'];//红包ID，用逗号分开
			$load['red'] = intval(strim($plan_temp['red']));//理财计划红包总额
			$load['total_money'] = intval(strim($plan_temp['total_money']));//理财计划红包总额
			$load['interestrate_id'] = intval($plan_temp['interestrate_id']);//加息卡ID
			$load['is_pc'] = 1;
			$load['user_id'] = $GLOBALS['user_info']['id'];
			$load['user_name'] = $GLOBALS['user_info']['user_name'];
			$load['load_time'] = time();
			$load['load_date'] = date("Y-m-d",time());
			$load['learn_id'] = 0;
			$load['repay_time'] = $plan['repay_time'];
			$GLOBALS['db']->query('update '.DB_PREFIX.'plan set load_money=load_money+'.$load['total_money'].' where id ='.$load['plan_id']);
			$res = $GLOBALS['db']->autoExecute(DB_PREFIX."plan_load",$load,"INSERT");
			$plan_load_id = $GLOBALS['db']->insert_id();
			$load['plan_load_id'] =$plan_load_id;
			
			/* $msg="[<a href='/index.php?ctl=plandeal&id=".$load['plan_id']."' target='_blank'>".$plan['name']."</a>]的出借";
            $brief = '出借成功';
            require_once APP_ROOT_PATH."system/libs/user.php";
            $data['cunguan_money'] = -($plan_temp['total_money']);
			$data['cunguan_lock_money'] = $plan_temp['total_money'];
            if($load['red']){//是否使用红包
				$data['red_money'] = -($load['red']);
				red_modify_account($data,$GLOBALS['user_info']['id'],$msg.',使用红包',1);
				unset($data['red_money']);
			}
			modify_account($data,$GLOBALS['user_info']['id'],$msg,2,$brief,1); */
			//增加抢红包次数
			 if($load['money']<=1000){
				insert_red_log($GLOBALS['user_info']['id'],6);
			}elseif($load['money']<=4999&&$load['money']>1000){
				insert_red_log($GLOBALS['user_info']['id'],7);
			}elseif($load['money']<=9999&&$load['money']>4999){
				insert_red_log($GLOBALS['user_info']['id'],8);
			}elseif($load['money']>=10000){
				insert_red_log($GLOBALS['user_info']['id'],9);
			} 
			$plan_all = $GLOBALS['db']->getOne("SELECT sum(total_money) FROM ".DB_PREFIX."plan_load where plan_id = ".$plan_temp["plan_id"]);
			if($plan_all>=$plan['borrow_amount']){//是否满标 
				$GLOBALS['db']->query('update '.DB_PREFIX.'plan set deal_status=2,success_time='.time().' where id='.$plan_temp["plan_id"]);
			}
			if(!$res){
				...
				$redis->select(6);
				$load['name'] = $plan['name'];
				$redis->lpush('plan_load',json_encode($load));
				$GLOBALS['db']->commit(); 
				// =============给用户发送短信通知=================
				/* if(app_conf("SMS_ON")==1){
					$user_info=$GLOBALS['db']->getRow("select user_name,mobile from ".DB_PREFIX."user where id=".$GLOBALS['user_info']['id']);
				...
				if(WAP==1){
					...
					es_session::delete('lend_money');
					es_session::delete('repay_time');
					es_session::delete('deal_type');
					//app_redirect(url("index","uc_invest#lendlist"));
					app_redirect(url("index","deal#success",array('plan_load_id'=>$plan_load_id)));
				}else{
					//showSuccess('恭喜您出借成功',0,url("index","uc_invest"));
					app_redirect(url("index","deal#plan_notice",array('plan_load_id'=>$plan_load_id)));
				}
			}
	}
	function plan_notice(){
		$user_id = intval($GLOBALS['user_info']['id']);
		if(!$user_id)
		{
			app_redirect(url("index","user#login"));		
		...
		$GLOBALS['tmpl']->assign('plan_load_id',$plan_load_id);
		$GLOBALS['tmpl']->display("plan_notice.html");
	}
	function plan_success(){
		$user_id = intval($GLOBALS['user_info']['id']);
		if(!$user_id)
		{
			app_redirect(url("index","user#login"));		
		}
		$plan_load_id= $_POST['plan_load_id'];
		$money = $GLOBALS['db']->getRow('select money,id from '.DB_PREFIX.'user_money_log where user_id='.$user_id.' and from_plan_load_id='.$plan_load_id.' limit 1');
		if($money){
			if(abs($money['money'])==0){
				$GLOBALS['db']->query('delete from '.DB_PREFIX.'user_money_log where id='.$money['id']);
			}
			if($GLOBALS['user_info']['pid']){
				/*$res = set_invite_cash_red_packet($uid,$pid,$bid_money,$repay_time); //奖励一 发放*/
				$timer = check_register_time($user_id); //注册时间小于15天
				if($timer){
					$firstLend = check_first_lend($user_id); //验证是否首次投资
					if($firstLend){
						$res1 = set_invite_cash_red_packet_two($user_id,$pid,$bid_money); //奖励二 发放
					}				
				}
				//验证出借人是否在当前月注册并投资  满足条件总数
				$count = check_register_lend_count($user_id,$pid);
				if($count == 2){
					$is_grant=is_grant($pid,30); //是否已经发放
					if($is_grant){
						$res2 = set_invite_lend_red_packet($user_id,$pid,30);
					}
				}else if($count == 4){
					$is_grant=is_grant($pid,70); //是否已经发放
					if($is_grant){
	...
	function dobid(){
		...
		$plan_id = $pc_con["plan_id"];
		$data['load_seqno'] = $pc_con["load_seqno"];
		$load_seqno = $pc_con["load_seqno"];
		 // print_r($plan_id);
		 // print_r($pc_con);die;
		if ($plan_id) {
			//=================PC跳过密码验证测试用==============
			// // $publics = new Publics();
			// $status = $this ->plan_dobid2($pc_con,$load_seqno);
			// // print_r($status);die;
			// if($status['status'] == 0){
			...
			// if($deal_load_data['cunguan_tag']==1&&WAP==0){
					$publics = new Publics();
			    	$html =  $publics ->verify_trans_password('deal','plan_dobid2',$GLOBALS['user_info']['id'],4,$data['load_seqno'],"_self");
			...
			$data['bid_money'] = strim($pc_con['bidmoney']);
	...
	// 理财计划 app验密
	function plan_check_pwd(){
		require APP_ROOT_PATH."system/utils/Depository/Require.php";
		$publics = new Publics();
		$load_seqno=$_REQUEST['load_seqno'];
		$html =  $publics ->verify_trans_password('deal','plan_dobid2',$GLOBALS['user_info']['id'],4,$load_seqno,"_self");
        echo $html;die;
	}
    ...
    function cg_dobid(){
        ...
        if($status['status']==1&&WAP==0){
			//增加抢红包次数
			if($data['bid_money']<=1000){
				insert_red_log($GLOBALS['user_info']['id'],6);
			}elseif($data['bid_money']<=4999&&$data['bid_money']>1000){
				insert_red_log($GLOBALS['user_info']['id'],7);
			}elseif($data['bid_money']<=9999&&$data['bid_money']>4999){
				insert_red_log($GLOBALS['user_info']['id'],8);
			}elseif($data['bid_money']>=10000){
				insert_red_log($GLOBALS['user_info']['id'],9);
			} 
        	// 给用户发送短信通知
            if(app_conf("SMS_ON")==1)
...
//		$data['bid_paypassword'] = MD5(trim(FW_DESPWD($user_data['bid_paypassword'])));
	...
	public function plus_interest(){
        ...
        // 投资可用红包
        //$red_list=get_lcinterest_card_list($limit,$user_id,$deal_id);
        $red_list=get_interest_card_list($limit,$user_id,$deal_id);
		 foreach($red_list['list'] as $k=>$v){
			if($v['use_time']>0){
                $red_list['list'][$k]['interest_time'] = "加息".$v['use_time']."天";
			}else{
                $red_list['list'][$k]['interest_time'] = "全程加息";
            }
    ...
    public function lcplus_interest(){
        $limit=' 0,1000';
        $user_id=$GLOBALS['user_info']['id'];
        $plan_id=intval($_REQUEST['plan_id']);
        // 投资可用红包
        $red_list=get_lcinterest_card_list($limit,$user_id,$plan_id);
        foreach($red_list['list'] as $k=>$v){
            $red_list['list'][$k]['begin_date']=date("Y-m-d",$v['begin_time']);
            $red_list['list'][$k]['end_date']=date("Y-m-d",$v['end_time']);
        }
...
//        $GLOBALS['tmpl']->display("page/experience_deal.html");
	...
	public function deals(){
		...
		es_session::set('deal_type',0); //1为普通项目
		/*获取session参数 输出到页面*/
    ...
    public function red_packet(){
		...
		$GLOBALS['tmpl']->assign("deal_type",es_session::get('deal_type'));
		...
		$limit ="0,1000";
		$deal = get_deal(es_session::get('deal_id'));
		if(isset($deal['plan_id'])){//如果有plan_id字段 说明是普通标 不是理财计划
			$red_list=get_red_list($limit,$user_id,$deal['id']);
		}else{
			$red_list=get_red_list($limit,$user_id,$deal['id'],true);

		}
		$red_packet = $red_list['list'];
		/* $red_packet = $GLOBALS['db']->getAll("select rp.id,rp.begin_time,rp.end_time,rp.content,rp.money,rpn.use_condition,rpn.amount,rpn.ratio,rpn.plan_day from ".DB_PREFIX."red_packet rp left join ".DB_PREFIX."red_packet_newconfig rpn on rp.red_type_id = rpn.id where rp.user_id = $user_id and rp.packet_type < 3 and rp.status = 0 and rp.end_time >".time()); */
    ...
    public function raise_interes(){
		...
		$deal = get_deal(es_session::get('deal_id'));
		if(isset($deal['plan_id'])){//如果有plan_id字段 说明是普通标 不是理财计划
			$raise_interes = get_interest_card_list($limit,$user_id,$deal['id']);
		}else{
			$deal = get_plandeal(es_session::get('deal_id'));
			$raise_interes = get_lcinterest_card_list($limit,$user_id,$deal['id']);
		...
		$raise_interes = $GLOBALS['db']->getAll("select i.*,c.use_condition,c.one_month,c.three_month,c.six_month,c.twelve_month,c.plan_day,c.interest_time from ".DB_PREFIX."interest_card i left join ".DB_PREFIX."coupon c on i.coupon_id = c.id where i.user_id = $user_id  and i.status = 0 and i.end_time >".time()." and c.".$repay."=1 order by i.end_time desc"); */
		foreach ($raise_interes['list'] as $k => $v) {
			if($v['use_time'] >0){
				$v['interest_time'] = "加息".$v['use_time']."天";
			}else{
