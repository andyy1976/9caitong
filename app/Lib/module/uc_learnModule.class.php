...
class uc_learnModule extends SiteBaseModule {
	...
	{	
        ...
        $time=time();
        $condition="AND use_status in(0,1) AND cunguan_tag=1";
        if ($voucher_paging > 0) {
            if ($voucher_paging == 1)
                $condition .= " AND use_status = 0 AND end_time >$time AND cunguan_tag=1";                
            else if ($voucher_paging == 2)
                $condition .= " AND use_status = 1  AND cunguan_tag=1";
        ...
        foreach($voucher_deal as $k=>$v){
            ...
            $voucher_deal[$k]['url'] = url("index","uc_learn#index",$tmp_args);  
        }
		...
		$GLOBALS['tmpl']->assign("page_title","体验金");
		*/
	...
	public function deal_invest(){
		...
		$GLOBALS['tmpl']->assign("page_title","体验金出借");
	...
	function dobid(){
		...
		if($bid_money > (round($deal['borrow_amount'],2) - round($deal['load_money'],2))){
			showErr("出借金额大于可投金额，投标失败！",$ajax,url("index","uc_learn#deal_invest"));
		}
