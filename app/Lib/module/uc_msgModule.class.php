...
class uc_msgModule extends SiteBaseModule
{
	...
	{	
		...
		if($mtype=="private"){
			...
			$GLOBALS['tmpl']->assign("page_title","我的消息");//通知
		...
		if($mtype == "private"){
			...
			$sql = "select * from ".DB_PREFIX."msg_box where is_delete=0 and to_user_id = ".$user_id." and `type` = 0 $extw ORDER BY create_time DESC limit ".$limit;
		...
		if($count > 0){
			...
			foreach($list as $k=>$v)
			{
				$list[$k] = $GLOBALS['db']->getRow("select *,mb.id as msg_id from ".DB_PREFIX."msg_box mb left join ".DB_PREFIX."msg_notice_type mnt on mb.is_notice=mnt.id where group_key = '".$v['group_key']."' and ((to_user_id = ".$user_id." and `type` = 0) or (from_user_id = ".$user_id." and `type` = 1))  order by create_time desc limit 1");
	...
	public function message_info(){
		...
		$message_info=$GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."msg_box mb left join ".DB_PREFIX."msg_notice_type mnt on mb.is_notice=mnt.id where mb.id=".$id);
		$GLOBALS['tmpl']->assign("msg_info",$message_info);
		$GLOBALS['tmpl']->assign("page_title","通知");
		$GLOBALS['tmpl']->assign("inc_file","inc/uc/uc_msg_details.html");
		$GLOBALS['tmpl']->display("page/uc.html");	
	}
