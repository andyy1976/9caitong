...
class articleModule extends SiteBaseModule
{
	...
	{			
		...
		if (!$GLOBALS['tmpl']->is_cached('page/article_index.html', $cache_id))	
		{
			...
			elseif($id==0&&$uname!='')
			{
				$id = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."article_cg where is_delete = 0 and is_effect = 1 and uname = '".$uname."'");
			}		
			...
			//开始输出当前的site_nav
			$cates = array();
			$cate = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."article_cate_cg where id = ".$article['cate_id']);
			do
			{
				$cates[] = $cate;
				$pid = intval($cate['pid']);
	...
	public function alist()
	{
		...
		if (!$GLOBALS['tmpl']->is_cached('page/article_list.html', $cache_id))	
		{
			...
			$cate = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."article_cate_cg where id = ".intval($_REQUEST['id']));
