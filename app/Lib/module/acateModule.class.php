...
class acateModule extends SiteBaseModule
{
	public function index()
	{
		...
		{		
			...
			$cate_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."article_cate_cg where id = ".$id." and is_effect = 1 and is_delete = 0");
			...
			elseif($cate_item['type_id']!=0)
			{
				...
				app_redirect(url("index","notice#list",array('cate_id'=>$id)));
			...
			$result = get_article_list($limit,$cate_id,$condition,'');
			...
			do
			{
				$cates[] = $cate;
				$pid = intval($cate['pid']);
				$cate = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."article_cate_cg where is_effect =1 and is_delete =0 and id = ".$pid);			
		...
		if($cate_item['type_id']==4){
		    $GLOBALS['tmpl']->display("page/video_list.html",$cache_id);
		}else{
