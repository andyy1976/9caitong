...
class goods_informationModule extends SiteBaseModule
{
	...
	public function doexchange(){
		...
		if($goods['cate_name']=="代金券"){
			$order_data['begin_time'] = TIME_UTC;
			$i = $goods['repay_time']-1; //代金券期限
			$end_time = strtotime("+$i day",TIME_UTC);
			$order_data['end_time'] = strtotime(date("Y-m-d 23:59:59",$end_time));
			$order_data['money'] = intval($goods['name']);
			$sn = unpack('H12',str_shuffle(md5(uniqid())));
			$ecv_list = $GLOBALS['db']->getRow("SELECT id,name FROM ".DB_PREFIX."ecv_type WHERE (begin_time= 0 or begin_time < ".TIME_UTC.") AND (end_time= 0 or end_time +24*3600-1 > ".TIME_UTC.") AND  send_type=1 ");
			$order_data['ecv_type_id'] = $ecv_list['id'];
			$order_data['content'] = $ecv_list['name'];
			$order_data['sn'] = $sn[1];
			$order_data['cunguan_tag'] = 1;
			$order_data['password'] = rand(10000000,99999999);
			if($ecv_list){
			$GLOBALS['db']->autoExecute(DB_PREFIX."ecv",$order_data,"INSERT");
			$order_data['order_status'] = 1;
			$order_data['delivery_time'] = TIME_UTC;
