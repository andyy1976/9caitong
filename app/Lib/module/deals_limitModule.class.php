...
class deals_limitModule extends SiteBaseModule
{
		...
		*/
		...
		{	
			...
			{		
				...
				$v['url'] = url("index","deals_limit",array("cid"=>$v['id']));
				$deal_cates[] = $v;
			}
			unset($deal_cates_db);
			//输出投标列表
			...
			$n_cate_id = 0;
			$condition = "publish_wait = 0 and is_effect = 0 and is_delete = 0 and cunguan_tag =1 and is_hidden = 0 and deal_status in(0,1,2,4,5)";
			...
			if(WAP == 1){
		    	...
		    	$deal_cate = $GLOBALS['db']->getAll("select id,name from ".DB_PREFIX."deal_loan_type where is_effect = 1 and is_delete = 0");
				foreach($deal_cate as $k=>$v){
					$tmp_args = $page_args;
					$tmp_args['deal_type'] = $v['id'];
					$deal_cate[$k]['url'] = url("index","deals_limit",$tmp_args);
				}
			...
			foreach($deal_time as $k=>$v){
				...
				$deal_time[$k]['url'] = url("index","deals_limit",$tmp_args);
			}
			...
			$result = get_deal_list($limit,$n_cate_id,$condition,$orderby,1,1,true);
			...
			foreach($result['list'] as $k=>$v){
				...
				if($v['debts']==1){
					...
					$result['list'][$k]['debts_repay_time']= (strtotime(date("Y-m-d",$last_repay_time))-strtotime(date("Y-m-d",time())))/3600/24;
				}
                if ($months_type > 0){
                    if ($months_type == 1)
                        $condition .= " AND ((repay_time = 1  and repay_time_type = 1) or repay_time_type = 0) ";
			...
			$tmp_args = $page_args;
			$tmp_args['cid'] = 0;
			$cate_list_url[0]['url'] = url("index","deals_limit#index",$tmp_args);
			...
			foreach($deal_cates as $k=>$v){
				...
				$tmp_args = $page_args;
				$tmp_args['cid'] = $v['id'];
				$cate_list_url[$k+1]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			...
			foreach($interest_url as $k=>$v){
				...
				$interest_url[$k]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			...
			foreach($lefttime_url as $k=>$v){
				...
				$lefttime_url[$k]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			...
			foreach($months_type_url as $k=>$v){
				...
				$months_type_url[$k]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			...
			foreach($months_type_deal as $k=>$v){
				...
				$months_type_deal[$k]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			...
			foreach($deal_status_url as $k=>$v){
				...
				$deal_status_url[$k]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			...
			$tmp_args = $page_args;
			$tmp_args['level'] = 0;
			$level_list_url[0]['url'] = url("index","deals_limit#index",$tmp_args);
			$level_list_url[0]['name'] = "不限";
			foreach($level_list['list'] as $k=>$v){
				$tmp_args = $page_args;
				$tmp_args['level'] = $v['id'];
				$level_list_url[$k+1] = $v;
				$level_list_url[$k+1]['url'] = url("index","deals_limit#index",$tmp_args);
			}
			$GLOBALS['tmpl']->assign('level_list_url',$level_list_url);
		
		    //  标的类型   车贷、房贷。。。
            $loan_types=$GLOBALS['db']->getAll("SELECT id,name  FROM ".DB_PREFIX."deal_loan_type where is_effect=1 and is_delete=0");
            array_unshift($loan_types,array('name'=>"不限"));

            foreach($loan_types as $k=>$v){
                $tmp_args = $page_args;
                $tmp_args['loan_type'] = $v['id'];
                $loan_types[$k]['url'] = url("index","deals_limit#index",$tmp_args);
            }
			...
			$loantype_url[0]['url'] = url("index","deals_limit#index",$tmp_args);
			...
			foreach($loantypes as $k=>$v){
				...
				$loantype_url[$v['loantype']+1]['url'] = url("index","deals_limit#index",$tmp_args);
			}
            ...
            if($temp_city_urls){
                ...
                $tmp_args = $page_args;
                $tmp_args['city'] = 0;
                $city_urls[0]['url'] = url("index","deals_limit#index",$tmp_args);
                ...
                foreach($temp_city_urls as $k=>$v){
                    if(isset($v['id'])){
                        ...
                        $tmp_args = $page_args;
                        $tmp_args['city'] = $v['id'];
                        $city_urls[$v['id']]['url'] = url("index","deals_limit#index",$tmp_args);
                    }
                }
            }
			...
			if($sub_citys){
				foreach($sub_citys as $k=>$v){
					...
					$tmp_args['city'] = $v['pid'];
					$tmp_args['scity'] = $v['id'];
					$sub_citys[$k]['url'] = url("index","deals_limit#index",$tmp_args);
				}
			}
            ...
            foreach($user_type_urls as $k=>$v){
                ...
                $user_type_urls[$k]['url'] = url("index","deals_limit#index",$tmp_args);
            }
		...
		$GLOBALS['tmpl']->display("page/deals_limit.html",$cache_id);
	}
	...
	public function productList(){
		...
		foreach($result['list'] as $k=>$v){
            ...
            if($progress>100){
				...
				$url_tmp_args['id'] = $v['id'];
				$result['list'][$k]['url'] = url("index","deals_limit#index",$url_tmp_args); 	//入口 控制器模块 控制器 参数数组
				$result['list'][$k]['rate'] = sprintf("%.1f",$v["rate"]);				//统一预期年化收益格式
				/*************朱湘************/
				//$result['list'][$k]['need_money'] = intval($v['need_money']);
                if($v['need_money'] > 10000){
