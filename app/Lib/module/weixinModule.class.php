...
class weixinModule extends SiteBaseModule
{
	...
	public function wx_auth(){
		...
		if (strpos($user_agent, 'MicroMessenger') === false) {
			...
			$user_info = $GLOBALS['db'] ->getRow("select * from ".DB_PREFIX."user where wx_openid=$unionid");
			// if(!es_session::is_set("user_info")){
   ...
   //              $userinfo = es_session::get("user_info");
			...
			if(es_session::get("user_info")['real_name']!==$user_info['real_name']){
   ...
   //              $userinfo = es_session::get("user_info");
	...
	public function wx_score(){
		...
		if (strpos($user_agent, 'MicroMessenger') === false) {
			...
			$unionid = $_REQUEST['unionid'];
			if(!$unionid){
				app_redirect(url("index","find#new_mall"));	
			}
			$user_info = $GLOBALS['db'] ->getRow("select * from ".DB_PREFIX."user where wx_openid=$unionid");
	...
	//$link：请求地址；$post_data：请求参数
	public function curl($link,$post_data){
		$ch = curl_init();
		// 2. 设置选项，包括URL
