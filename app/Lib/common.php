...
/* 				if(($v['u_module']=='deals' || $v['u_module']=='transfer' || $v['u_module']=='tool') && MODULE_NAME==$v['u_module']){
				...
				//当前分类高亮使用。上面处理有局限，并且处理了子类   by zhangteng 20160828
				if($nav_list[$k]['url'] == $_SERVER['REQUEST_URI']){
					$nav_list[$k]['current'] = 1;
				}
...
function get_notice($limit=0)
{
	...
	$list = $GLOBALS['db']->getAll("select a.*,ac.type_id from ".DB_PREFIX."article_cg as a left join ".DB_PREFIX."article_cate_cg as ac on a.cate_id = ac.id where ac.type_id = 2 and ac.is_effect = 1 and ac.is_delete = 0 and a.is_effect = 1 and a.is_delete = 0 order by a.sort desc ".$limit_str);
...
//===========wap2.0平台公告================
function get_mobile_notice($limit=0)
{
	if($limit == 0)
	$limit = app_conf("INDEX_NOTICE_COUNT");
	if($limit>0)
	...
	$list = $GLOBALS['db']->getAll("select a.*,ac.type_id from ".DB_PREFIX."app_msg as a left join ".DB_PREFIX."app_msg_cate as ac on a.cate_id = ac.id where ac.type_id = 2 and ac.is_effect = 1 and ac.is_delete = 0 and a.is_effect = 1 and a.is_delete = 0 order by a.sort desc ".$limit_str);
...
function get_mobile_article($id)
{
	return $GLOBALS['db']->getRow("select a.*,ac.type_id from ".DB_PREFIX."app_msg as a left join ".DB_PREFIX."app_msg_cate as ac on a.cate_id = ac.id where a.id = ".intval($id)." and a.is_effect = 1 and a.is_delete = 0");
}
/**
 * 获取文章列表
 */
function get_mobile_article_list($limit, $cate_id=0, $where='',$orderby = '',$cached = true)
{		
		...
		if($res===false)
		{
			...
			$count_sql = "select count(*) from ".DB_PREFIX."app_msg as a left join ".DB_PREFIX."app_msg_cate as ac on a.cate_id = ac.id where a.is_effect = 1 and a.is_delete = 0 and ac.is_delete = 0 and ac.is_effect = 1 ";
			$sql = "select a.*,ac.type_id,ac.icon from ".DB_PREFIX."app_msg as a left join ".DB_PREFIX."app_msg_cate as ac on a.cate_id = ac.id where a.is_effect = 1 and a.is_delete = 0 and ac.is_delete = 0 and ac.is_effect = 1 ";
...
//wap banner列表
function get_wap_nav($type)
{
	if($type == "index"){
		$result = $GLOBALS['db']->getAll(" select id,app_page,img,name,type,url from ".DB_PREFIX."app_nav_cg where is_effect =1 and device != 2 and UNIX_TIMESTAMP(add_time) < ".TIME_UTC." and UNIX_TIMESTAMP(end_time) > ".TIME_UTC." order by sort desc ");
	}else{
		$result = $GLOBALS['db']->getAll(" select id,app_page,img,name,type,url from ".DB_PREFIX."app_fx_nav_cg where is_effect =1 and device!=2 and UNIX_TIMESTAMP(add_time) < ".TIME_UTC." and UNIX_TIMESTAMP(end_time) > ".TIME_UTC." order by sort desc ");
	}
	...
	foreach ($result as $key => $val) {
		if($val['type'] == 1){
			...
			switch ($val['app_page']) {
				case '1':
					$val['url'] = SITE_DOMAIN."/index.php";
					break;
				case '2':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=deals";
					break;
				case '3':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=find";
					break;
				case '4':
					$val['url'] = SITE_DOMAIN."/member.php?ctl=uc_center";
					break;
				case '5':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=user&act=register";
					break;
				case '6':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=user&act=login";
					break;
				case '7':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=invite";
					break;
				case '8':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=score&act=earn_points";
					break;
				case '9':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=score";
					break;
				case '10':
					$val['url'] = SITE_DOMAIN."/member.php?ctl=uc_set&act=news";
					break;
				case '11':
					$val['url'] = SITE_DOMAIN."/member.php?ctl=uc_set&act=help";
					break;
				case '12':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=user&act=login";
					break;					
...
//wap栏目列表
function get_wap_cloumn($type)
{
	if($type == "index"){
		$result = $GLOBALS['db']->getAll(" select * from ".DB_PREFIX."app_cloumn_cg where clo_type = 'index' order by code ASC ");
	}else{
		$result = $GLOBALS['db']->getAll(" select * from ".DB_PREFIX."app_cloumn_cg where clo_type = 'find' order by code ASC ");
	}	
	foreach ($result as $key => $val) {
		if($val['type'] == 1){
			$val['url'] = $val['url'];
			...
			switch ($val['app_page']) {
				...
				case '6':
					...
					break;
				case '7':
					//$val['url'] = "/index.php?ctl=invite";
					$val['url'] = SITE_DOMAIN."/index.php?ctl=welfare";
					break;
				case '8':
					$val['url'] = SITE_DOMAIN."/index.php?ctl=score&act=earn_points";
					break;
...
//wap栏目列表
function get_activity_cloumn($type)
{
	if($type == "1"){
		$result = $GLOBALS['db']->getAll("select id,title,end_time,app_page,img,name,type,url,act_type from ".DB_PREFIX."app_activity_cg where act_type = 1 and is_effect =1 and UNIX_TIMESTAMP(add_time) < ".TIME_UTC." and UNIX_TIMESTAMP(end_time) > ".TIME_UTC." order by sort desc");
	}elseif($type == "2"){
		$result = $GLOBALS['db']->getAll("select id,title,app_page,img,name,type,url,act_type from ".DB_PREFIX."app_activity_cg where act_type = 2 and device != 2 and is_effect =1 and UNIX_TIMESTAMP(add_time) < ".TIME_UTC." order by sort desc");
	}else{
        $result = $GLOBALS['db']->getAll("select id,title,end_time,app_page,img,name,type,url,act_type from ".DB_PREFIX."app_activity_cg where act_type = 1 and is_effect =1  and UNIX_TIMESTAMP(end_time) < ".TIME_UTC." order by sort desc limit 0,10");	}
	foreach ($result as $key => $val) {
		if($val['type'] == 1){
			$val['url'] = $val['url'];
		}elseif($val['type'] == 2){
			switch ($val['app_page']) {
				case '1':
					...
					break;
				case '2':
					$val['url'] = "/index.php?ctl=deals";
					break;
				case '3':
					$val['url'] = "/index.php?ctl=find";
					break;
				case '4':
					$val['url'] = "/member.php?ctl=uc_center";
					break;
				case '5':
					$val['url'] = "/index.php?ctl=user&act=register";
					break;
				case '6':
					$val['url'] = "/index.php?ctl=user&act=login";
					break;
				case '7':
					$val['url'] = "/index.php?ctl=invite";
					break;
				case '8':
					$val['url'] = "/index.php?ctl=score&act=earn_points";
					break;
				case '9':
					$val['url'] = "/index.php?ctl=score";
					break;
				case '10':
					$val['url'] = "/member.php?ctl=uc_set&act=news";
					break;
				case '11':
					$val['url'] = "/member.php?ctl=uc_set&act=help";
					break;
				case '12':
					$val['url'] = "/index.php?ctl=user&act=login";
					break;					
...
{		
		...
		if($res===false)
		{
			...
			$count_sql = "select count(*) from ".DB_PREFIX."article_cg as a left join ".DB_PREFIX."article_cate_cg as ac on a.cate_id = ac.id where a.is_effect = 1 and a.is_delete = 0 and ac.is_delete = 0 and ac.is_effect = 1 ";
			$sql = "select a.*,ac.type_id from ".DB_PREFIX."article_cg as a left join ".DB_PREFIX."article_cate_cg as ac on a.cate_id = ac.id where a.is_effect = 1 and a.is_delete = 0 and ac.is_delete = 0 and ac.is_effect = 1 ";
...
{
	return $GLOBALS['db']->getRow("select a.*,ac.type_id from ".DB_PREFIX."article_cg as a left join ".DB_PREFIX."article_cate_cg as ac on a.cate_id = ac.id where a.id = ".intval($id)." and a.is_effect = 1 and a.is_delete = 0");
}
function get_article_buy_uname($uname)
{
	return $GLOBALS['db']->getRow("select a.*,ac.type_id from ".DB_PREFIX."article_cg as a left join ".DB_PREFIX."article_cate_cg as ac on a.cate_id = ac.id where a.uname = '".addslashes($uname)."' and a.is_effect = 1 and a.is_delete = 0");
}
 ...
 * @param user $user_id 用户id
 * @return float 提现手续费
 */
function getWithdrawFee($money,$user_id){
	$paymentMOney=$GLOBALS['db']->getOne("SELECT sum(money) FROM ".DB_PREFIX."payment_notice where is_paid =1 and user_id=".$user_id);
	$dealMOney=$GLOBALS['db']->getOne("SELECT sum(money) FROM ".DB_PREFIX."deal_load where user_id=".$user_id);
	if($paymentMOney - $dealMOney >= $money)
		$fee =$money * 0.005;
	else if($paymentMOney - $dealMOney  < $money && $paymentMOney - $dealMOney > 0)
		$fee =$paymentMOney - $dealMOney * 0.005;
	else if($paymentMOney - $dealMOney  <= 0)
		$fee = 0;
	return $fee;
}
/**
 ...
 */
 function site_statics(){
	 /*$redis = new Redis();
      ...
      $redis->select(5);
	  $stats = json_decode($redis->get(REDIS_PREFIX.'site_statics'),true);
	  if(!$stats){*/
		  //出借金额和出借笔数
		  $load_rs = $GLOBALS['db']->getRow("SELECT sum(money) as total_load,count(*) as load_count FROM ".DB_PREFIX."deal_load where is_repay=0 ");
		  //注册人数
		  $reg_count['user_count'] =$GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."user where is_effect = 1");
		  $arr =array_merge($load_rs,$reg_count);
		  $arr = json_encode($arr);
		 /* $redis->setex(REDIS_PREFIX.'site_statics',600,$arr);*/
	  /*}*/
...
/*function site_statics(){
	...
	if($stats==false){	
		//累计出借金额
		$load_rs = $GLOBALS['db']->getRow("SELECT sum(money) as total_load,count(*) as load_count FROM ".DB_PREFIX."deal_load where is_repay= 0 ");
		//$stats['total_load'] = $load_rs['total_load'] + floatval(trim(app_conf("VIRTUAL_MONEY_1")));//虚拟的累计成交额
		$stats['total_load'] = $load_rs['total_load'];//虚拟的累计成交额
	...
	//	$statsU = $GLOBALS['db']->getRow("SELECT sum(AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."')) as total_usermoney ,count(*) total_user FROM ".DB_PREFIX."user where is_effect = 1 and is_delete=0 ");
		...
		//注册人数
		//$stats['user_count'] = $GLOBALS['db']->getOne("SELECT count(1) FROM ".DB_PREFIX."user where is_effect = 1 and user_type in(0,1)");
		$stats['user_count'] = "1142316";
...
function signin($user_id){
	...
	if($t_sign_data)
	{
		...
		if($point>0||$score>0||$money>0||$nmc_money>0)
		{
			...
			$resultSignLog = $GLOBALS['db']->autoExecute(DB_PREFIX."user_sign_log",$sign_log);	
			if($resultSignLog){
				$result['status'] = 1;
				$result['info'] = "签到成功";	
			}else{
...
/**
 * 判断注册用户的邀请人是否达到发放奖励的条件
 */
function is_get_rewards($user_id){
	$user_bank_sql="SELECT u.idcardpassed,u.pid,u.create_time as r_create_time,ub.create_time,u.pid,u.referer FROM ".DB_PREFIX."user as u LEFT JOIN ".DB_PREFIX."user_bank as ub on ub.user_id=u.id where ub.status =1 and u.id=$user_id";
 ...
 * 获取指定用户获取邀请奖励的次数
 */
function get_rewards_times($id){
	require APP_ROOT_PATH.'app/Lib/uc.php';
	$friends_info = get_invite_list(51,$id,3);
	if(!empty($friends_info['list'])){
		foreach($friends_info['list'] as $k=>$v){
			$fri_data=$GLOBALS['db']->getRow("SELECT u.create_time as r_create_time,ub.create_time,u.pid,u.referer FROM ".DB_PREFIX."user u LEFT JOIN ".DB_PREFIX."user_bank ub on u.id=ub.user_id where u.id=".$v['id']);
			if(ceil(($fri_data['create_time']-$fri_data['r_create_date'])/3600/24)>30){
				$data[]=$fri_data;
			}
		}
		return count($data);
...
function jumpUrl($key){
	$http_type = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https')) ? 'https://' : 'http://'; 
	$jumpUrl = $http_type . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
    es_cookie::set($key,$jumpUrl);
}
//处理负载均衡下无法获取真实用户IP  added by zhangteng 20160907
function get_nginx_client_ip($type = 0,$adv=false) {
    $type       =  $type ? 1 : 0;
    static $ip  =   NULL;
...
//渠道统计方法
/* function addsource($source_id,$device,$user_id){
	$data['source_id'] = intval($source_id);
	$data['device'] = strim($device);
	$data['add_time'] = TIME_UTC;
	$data['user_id'] = intval($user_id);
	$data['ip'] = get_client_ip();
	$GLOBALS['db'] ->autoExecute(DB_PREFIX."statistics",$data,"INSERT");
	return true;
} */
function addsource($source_id,$device,$user_id,$type=0,$tag=0){
	$user = $GLOBALS['db'] ->getRow("select * from ".DB_PREFIX."statistics where user_id=".$user_id);
	if($user['user_id']>0){
		if($type==1){
			$data['register_time'] = TIME_UTC;
			//$data['register_date'] = date("Y-m-d H:i:s");
		}elseif($type==2){
			if(empty($user['cg_bindcard_time'])){
			$data['cg_bindcard_time'] = TIME_UTC;
			}
			//$data['bindCard_date'] = date("Y-m-d H:i:s");
		}elseif($type==3){
			if(empty($user['cg_bid_time'])){
				$data['bid_time'] = TIME_UTC;
			}
			
			//$data['bid_date'] = date("Y-m-d H:i:s");
		}elseif($type==4){
			if(empty($user['cg_account'])){
				$data['cg_account'] = TIME_UTC;
			}
			
			//$data['bid_date'] = date("Y-m-d H:i:s");
		}
		$GLOBALS['db'] ->autoExecute(DB_PREFIX."statistics",$data,"update","user_id =".$user_id);
	}else{
		...
		if($source_id){
			$data['add_time'] = TIME_UTC;
			$data['source_id'] = strim($source_id);
			$data['device'] = strim($device);
			$data['user_id'] = intval($user_id);
			$data['ip'] = get_client_ip();
			$data['cunguan_tag'] = $tag;
			$GLOBALS['db'] ->autoExecute(DB_PREFIX."statistics",$data,"INSERT");
		}
...
/*邀请好友 奖励一：年化佣金
* 1、年化佣金0.3%，佣金费率可进行自由调配；
* 2、年化佣金=年化投资额*佣金费率（0.3%）；
* 3、佣金以现金红包形式自动发放至用户账户，可直接提现，无手续费；
* 4、暂定邀请一级享活动奖励；
* recommender_id 推荐人id   uid 投资人id
* money 投资金额 term 投资期限
*/
function set_invite_cash_red_packet($uid,$recommender_id,$money,$term){
	require_once APP_ROOT_PATH."system/libs/voucher.php";
	//邀请好友 奖励一
	$red_id = $GLOBALS['db']->getOne("SELECT id FROM ".DB_PREFIX."red_packet_newconfig WHERE grant_type = 3");
	$foldMoney =  $money * $term /12; // 折标金额
	$red_money = sprintf("%.2f",$foldMoney * 0.3/100);
	$res = red_packet($red_id,$recommender_id,$msg,$red_money);
	if($res){
		return true;
	}else{
		return false;
...
* 被邀请人投资  邀请人获得
* 1000≤投资金额<5000   15元
* 5000≤投资金额<10000  15元+25元
* 投资金额≥10000        15元+25元+40元
* 1、被邀请人15天内投资以上对应额度可享对应现金红包；
* 2、被邀请人仅首次投资，邀请人才可享；
* 3、现金奖以现金红包形式发放，可直接提现，无手续费；
* 4、暂定一级享活动奖励；
* 5、投资金额此项参数可自由配置。
*/
function set_invite_cash_red_packet_two($uid,$recommender_id,$money){
	require_once APP_ROOT_PATH."system/libs/voucher.php";
	$red_id = $GLOBALS['db']->getOne("SELECT id FROM ".DB_PREFIX."red_packet_newconfig WHERE grant_type = 3");
	$cash_red_packet =array(
		array('start' => '1000','end' => '5000','money'=>'15'),
		array('start' => '5000','end' => '10000','money'=>'40'),
		array('start' => '10000','end' => '100000000','money'=>'80'),
	);
	foreach ($cash_red_packet as $k => $v) {
		if($money >= $v['start'] && $money < $v['end']){
			$res = red_packet($red_id,$recommender_id,$msg,$v['money']);
		}
	}
	if($res){
		return true;
...
* 1、此邀请按自然月计算
* 2、每月最高得100元；
* 3、被邀请人投资满1000元邀请人邀请成功；
* 4、邀请人数及获得奖励金额可自由调配；
* 5、奖励以出借红包形式发放，红包面值不做拆分。
**/
function set_invite_lend_red_packet($uid,$recommender_id,$money){
	require_once APP_ROOT_PATH."system/libs/voucher.php";
	$red_id = $GLOBALS['db']->getOne("SELECT id FROM ".DB_PREFIX."red_packet_newconfig WHERE grant_type = 2");
	$res = red_packet($red_id,$recommender_id,$msg,$money);
	if($res){
		return true;
	}else{
		return false;
...
//验证邀请奖励三是否已经发放
function is_grant($recommender_id,$money){
	if(!$recommender_id) return false;
	$begin_month = strtotime(date("Y-m-1")); 		//本月开始时间
	$end_month = strtotime("+1 month",$begin_month); //本月开始时间
	$is_grant = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."red_packet where user_id=".$recommender_id." and create_time >=".$begin_month." and create_time <".$end_month." and money =".$money);
	if($is_grant > 0) 
		return false;
	else 
		return true;
...
//验证出借人是否在当前月注册并投资总数
function check_register_lend_count($uid,$recommender_id){
	$begin_month = strtotime(date("Y-m-1")); 		//本月开始时间
	$end_month = strtotime("+1 month",$begin_month); //本月开始时间
	//查询同一推荐人满足条件人数
	$friend_list = $GLOBALS['db']->getAll("select u.id,sum(dl.money) as money from ".DB_PREFIX."user u inner join ".DB_PREFIX."deal_load dl on u.id=dl.user_id  where u.pid =".$recommender_id." and u.create_time >=".$begin_month." and u.create_time <".$end_month." group by u.id");
...
//验证投资人是否注册时间是否超过15天
function check_register_time($uid){
	$create_time = $GLOBALS['db']->getOne("select create_time from ".DB_PREFIX."user where id=".$uid); // 查询注册时间
	$timer = intval((time() - $create_time)/24/60/60);
	if($timer <= 15){
		return true;
	}else{
		return false;
...
//验证投资人是否首次投资
function check_first_lend($uid){
	$sum = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."deal_load where user_id=".$uid); // 查询注册时间
	if($sum == 1){
		return true;
	}else{
		return false;
...
function insert_red_log($user_id,$type){
		$t_begin_time = to_timespan(to_date(TIME_UTC,"Y-m-d"));  //今天开始
		$t_end_time = to_timespan(to_date(TIME_UTC,"Y-m-d"))+ (24*3600 - 1);  //今天结束
		$red_log = $GLOBALS['db']->getOne('select count(id) from '.DB_PREFIX.'user_red_log where user_id='.$user_id.' and create_time>='.$t_begin_time.' and create_time<='.$t_end_time.' and type='.$type.' limit 1');
		$data['user_id']=$user_id;
		$data['type']=$type;
		$data['create_time']=TIME_UTC;
		switch($type){
			...
			*/
			...
			if(!$red_log){
				$data['increase'] = 1;
				$GLOBALS['db']->autoExecute(DB_PREFIX.'user_red_log',$data,"INSERT");
			}
			break;
			case 4://积分商城兑换
			if(!$red_log){
