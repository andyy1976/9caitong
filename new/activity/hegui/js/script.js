...
//轮播图-------------------------------------------------------------------------------------
	...
	function slide(slideMode){//轮播方法		
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				hidden.css({width:"410px",height:"260px",top:"50px",left:"0px","opacity":0});
				hidden.stop(true,true).animate({width:"410px",height:"260px",top:"50px",left:"0px",opacity:1},timer);
				curr.stop(true,true).animate({width:"410px",height:"260px",top:"50px",left:"600px",opacity:1},timer);
				next.stop(true,true).animate({width:"410px",height:"260px",top:"50px",left:"0px",opacity:0},timer,function(){
					next.find("span").css("opacity",0); isMoving = false;
				});
				//prev  -->  curr     prev中的图片li轮换到curr的位置      其他一次轮换
				prev.find("span").css("opacity",0);
				prev.stop(true,true).animate({width:"580px",height:"360px",left:"200px",top:0,opacity:1},timer);
				//给各个li加控制图片和p样式的class
				curr.attr('class','second');
				next.attr('class','second');		
				hidden.attr('class','second');
				prev.attr('class','first');
				index_2--;
			}else{			//执行next 操作
				...
				hidden.css({width:"410px",height:"260px",top:"50px","left":"600px","opacity":0});
				hidden.stop(true,true).animate({width:"410px",height:"260px",top:"50px",left:"600px",opacity:1},timer);							
				curr.stop(true,true).animate({width:"410px",height:"260px",top:"50px",left:"0px",opacity:1},timer);
				//next  -->  curr     next中的图片li轮换到curr的位置      其他一次轮换
				next.find("span").css("opacity",0);
				next.stop(true,true).animate({width:"580px",height:"360px",left:"200px",top:0,opacity:1},timer);
				prev.stop(true,true).animate({width:"410px",height:"260px",left:"600px",top:"50px",opacity:0},timer,function(){
					isMoving = false;
				});
				curr.attr('class','second');
				next.attr('class','first');		
				hidden.attr('class','second');
				prev.attr('class','second');
				index_2++;	
			}//if else
	...
	if(len>3){
		...
		$(".main_banner_box li").on("mousemove",function(){
			if($(this).css("width")=="580px"){//鼠标移入为当前正中显示的图片li，则清除定时器
				clearInterval(intervaltimer);
				intervaltimer=null;
			}
