...
function to_time($utc_time, $format = 'Y-m-d H:i:s'){
    if (empty ( $utc_time )) {
        return '';
    }
...
//处理负载均衡下无法获取真实用户IP  added by wanagwenming 20170504
...
function syn_deal_status($id)
{
	...
	if($deal_info['deal_status']!=3){
		if($deal_info['progress_point'] <100){
			$data['load_money'] = $GLOBALS['db']->getOne("SELECT sum(total_money) FROM ".DB_PREFIX."deal_load  WHERE deal_id=$id ");
		...
		if(($deal_info['progress_point'] >=100 || $data['progress_point'] >=100) && floatval($deal_info['load_money']) >=floatval($deal_info['borrow_amount'])){
			...
			if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_inrepay_repay WHERE deal_id=$id") >0){//提前还款 这个表里面有没有这个值
				$data['deal_status'] = 5;
				...
				if($repay_info){
					$data['repay_money'] = $repay_info['all_repay_money'];//真实还款本息
				}
                //还款时间   提前还款是否还了
	...
	//投标人数
	$sdata = $GLOBALS['db']->getRow("SELECT count(*) as buy_count,sum(total_money) as load_money FROM ".DB_PREFIX."deal_load WHERE deal_id=$id");
...
//	}
	...
	if($deal_info['deal_status']==4) {
		...
		if ($deal_info['debts'] == 1) {
			$where = $deal_info['old_deal_id'];
		} else {
			$where = $id;
		}
		if ($GLOBALS['db']->getOne("SELECT count(*) FROM " . DB_PREFIX . "deal_repay WHERE has_repay<>1 AND deal_id=$where ") == 0) {
			$data['deal_status'] = 5;
		} else {
			$data['deal_status'] = 4;
		}
	...
	//自动投标功能---
	// 排除了   借贷者在外 and usa.user_id <>'".$deal_info['user_id']."'
...
function make_repay_plan($deal){
	...
	if($deal['cunguan_tag']==1){//判断是否是存管标的
		require_once APP_ROOT_PATH."system/utils/Depository/Require.php";
		$publics = new Publics();
		$seqno = $publics ->seqno();
		$deals = new Deal();
		$deal_info = $GLOBALS['db']->getRow("select user_id,objectaccno,borrow_amount from ".DB_PREFIX."deal where id = ".$deal['id']);
		$user_info =$GLOBALS['db']->getRow("select accno from ".DB_PREFIX."user where id = ".$deal_info['user_id']);
		$data['deal_id'] = (string)$deal['id'];
		$money = floatval($deal_info['borrow_amount']);
		$objectaccno = $deal_info['objectaccno'];
		$accno = $user_info['accno'];
		$data['accountList'] = array(array("oderNo"=>"1","oldbusinessSeqNo"=>"","oldOderNo"=>"","debitAccountNo"=>$objectaccno,"cebitAccountNo"=>$accno,"currency"=>"CNY","amount"=>$money,"otherAmounttype"=>"03","otherAmount"=>"0.00"));
		$data['accNo'] =$accno;
		$data['objectaccNo'] =$deal_info['objectaccno'];
		$repay = $deals -> do_repay($seqno,'T03',$data);//标的放款
		 /*  $repay['seqno'] =$seqno;
		var_dump($repay); die;    */
		$repay_code = $repay['respHeader']['respCode'];
		if($repay_code=="P2P0000"){
			$seqno1 = $publics ->seqno();
			$res = $deals -> save_deal($deal['id'],$seqno1,"03",$list);//修改标的状态为放款
			$res_code = $res['respHeader']['respCode'];
			  /* $res['seqno'] =$seqno1;
			var_dump($res);die;   */
			if($res_code=="P2P0000"){
				$seqno2 = $publics ->seqno();
				$res1 = $deals -> save_deal($deal['id'],$seqno2,"06",$list);//修改标的状态为还款中
				...
				if($res_code=="P2P0000"){
					...
					modify_account(array("cunguan_money"=>$deal['borrow_amount'],"deal_id"=>$deal['id']),$deal['user_id'],"放款成功",3,"存管标的放款",1);
				}
			}
		}else{
			return false;
...
function deal_repay_money($deal){
	$loantype = intval($deal['loantype']);
	$LoanModule = LoadLoanModule($loantype);//去查还款方式里面的文字
	return $LoanModule->deal_repay_money($deal); //返回这个方法的数据
}
...
/*
 * 推广群发短信走的是凌讯中科短信平台*/
function send_lbsms_email($msg_item){
	$re = array('status'=>0,'msg'=>'');
	if($msg_item['send_type']==0)
	{
		//短信
		require_once APP_ROOT_PATH."system/utils/es_lb_sms.php";
		$sms = new sms_sender();
...
function evc_money($price){
    return $price*50;
}

function format_price_trim($price,$decimals=2){
    return app_conf("CURRENCY_UNIT")."".ltrim(number_format($price,2),"-");
}

function format_price_money($price,$decimals=2){
	return intval($price);
}

function red_type($type){
    switch($type){
        case 0:
            echo $types ="结存";
            break;
...
function mobile_substr($str){
	...
	if(count($t_string[0])==2)
		...
		return cut_str($str, 3, 0).'****'.cut_str($str, 4, -4);
}
...
function url($app_index,$route="index",$param=array()){
	...
	if (isset($GLOBALS['request']) && $GLOBALS['request']['from']!="wap"){//PC跳转
		if(isset($GLOBALS[$key]))
		{
			$url = $GLOBALS[$key];//包括当前的数组
			return $url;
		}
	...
	if(app_conf("URL_MODEL")==0 || $GLOBALS['request']['from']=="wap")
	{
		...
		if($GLOBALS['request']['from']=="wap"){
			if(strpos(APP_ROOT,"/wap")===false)
				$url = SITE_DOMAIN.APP_ROOT."/wap"."/".$app_index.".php";
			else
				$url = SITE_DOMAIN.APP_ROOT."/".$app_index.".php";
		}
		else
			$url = SITE_DOMAIN.APP_ROOT."/".$app_index.".php";
...
function do_loans($id,$repay_start_time,$type = 0){
    ...
    $res1 = $GLOBALS['db']->autoExecute(DB_PREFIX."deal",$loan_data,"UPDATE","id=".$id." AND is_has_loans=0 ");
    $deal_info['repay_start_time'] = $loan_data['repay_start_time'];

    if($res1){
        //生成募集期收益
        $load_list_one = $GLOBALS['db']->getAll("select id,create_time,total_money from " . DB_PREFIX . "deal_load where deal_id = ".intval($id)."");
        $raise_money = 0;
        foreach ($load_list_one as $k => $v) {
            $user_raise_time = (strtotime(date("Y-m-d",$deal_info['repay_start_time']))-strtotime(date("Y-m-d",$v['create_time'])))/86400;
            $user_raise_money = round($v['total_money'] * $deal_info['rate'] / 100 / 365 * $user_raise_time, 2);
            $GLOBALS['db']->query("UPDATE " . DB_PREFIX . "deal_load SET raise_money=" . $user_raise_money . " WHERE id=" . $v['id'] . "");
            $raise_money += $user_raise_money;
        }
        $deal_info['raise_money'] = $raise_money;
                 ...
                 if($deal_info['repay_time_type'] == 0)
                     $end_repay_time =  $deal_info['repay_start_time'] + $end_repay_time['repay_time']*24*3600;
                         ...
                         if(floatval($lv["rebate_money"])!=0)
                             modify_account(array("money"=>$lv['rebate_money']),$lv['user_id'],"[<a href='".$deal_info['url']."' target='_blank'>".$deal_info['name']."</a>],出借返利",24);
                             //积分
                             if(intval($lv["bid_score"])!=0)
                                 modify_account(array("score"=>$lv['bid_score']),$lv['user_id'],"[<a href='".$deal_info['url']."' target='_blank'>".$deal_info['name']."</a>],出借返积分",2);
        ...
        */
        ...
        $load_list_two = $GLOBALS['db']->getAll("select id,create_time,total_money,user_id,raise_money from " . DB_PREFIX . "deal_load where deal_id = ".intval($id)."");
        foreach($load_list_two as $k=>$v){
            // 生成满标放款站内信
            $notices['site_name'] = app_conf("SHOP_TITLE");
            $notices['user_name'] = get_user_info("real_name","id=".$v['user_id'],"ONE");
            $notices['invest_time'] =to_date($v['create_time']);
            $notices['invest_name'] =$deal_info['name'];
            $notices['project_repay'] =round($v['total_money']*$deal_info['rate']/100/12*$deal_info['repay_time'],2);
            $notices['raise_money']=$v['raise_money'];
            $to_user_id=$v['user_id'];
            $time=TIME_UTC;
            $tmpl_contents = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."msg_template where name = 'TPL_INS_FULL_INVEST_REPAY'",false);
            $GLOBALS['tmpl']->assign("notice",$notices);
            $content = $GLOBALS['tmpl']->fetch("str:".$tmpl_contents['content']);
            send_user_msg("您出借的".$deal_info['name']."已经成功满标放款给借款人。",$content,0,$to_user_id,$time,0,true,23);
            ///////////////////////////////////
        }
        ...
        /*
         ...
         $msg_item['dest']=$mobile;
         $msg_item['content']="尊敬的玖财通用户，您所借的".$deal_info['load_money']."元资金已放款至您的玖财通账户，请尽快登录玖财通平台提现；详情可拨打010-53608032咨询";
         require_once APP_ROOT_PATH."system/utils/es_sms.php";
         $sms = new sms_sender();
         $result = $sms->sendSms($msg_item['dest'],$msg_item['content']);
...
function hideIdCard($idcard){
	 if($idcard!="")
	 	return preg_replace('#(\d{14})\w{4}|(\w+)#', '${1}****',$idcard);
	 else
	 	return "";

}
function hideIdnoCard($idcard){
	 if($idcard!="")
	 	return preg_replace('#(\d{3})\d{11}(\d{4})#', '${1}***********${2}',$idcard);
	 else
	 	return "";
...
function FW_DESPWD($pwd){
	...
	$pwd = substr($pwd,strlen(LOGIN_DES_KEY()."玖财通"),strlen($pwd));
	$pwd = substr($pwd,0,strlen($pwd) - strlen("理财"));
	return $pwd;
}
 ...
 * 理财计划投标数据格式化
 */
function plan_return_deal_load_data($data,$user_info,$deal){
	$formatdata = array();
	...
	$formatdata['cunguan_tag'] = $data['cunguan_tag'];
	$formatdata['add_ip'] = $data['add_ip'];
	//$formatdata['create_time'] = TIME_UTC;
	//$formatdata['create_date'] = to_date(TIME_UTC);
	$formatdata['is_auto'] = intval($data['is_auto']);
	//$formatdata['plan_load_id'] = $data['plan_load_id'];
	$formatdata['rate'] = $data['rate'];
	$formatdata['plan_id'] = $data['plan_id'];
	$formatdata['repay_time'] = $data['repay_time'];
	$formatdata['load_time'] = TIME_UTC;
	$formatdata['update_time'] = TIME_UTC;
	$formatdata['load_date'] = to_date(TIME_UTC);	
	$formatdata['total_money'] = $data['total_money'];
	$formatdata['red'] = $data['red'];
	$formatdata['ecv_id'] = $data['plan_red_id'];
	$formatdata['ecv_money'] = $data['ecv_money']; //代金券
	$formatdata['interestrate_money'] = floatval($data['plan_interestrate_money']);
...
function return_deal_load_data($data,$user_info,$deal){
	...
	*/
	$formatdata['ecv_id'] = $data['ecv_id'];
	$formatdata['ecv_money'] = $data['ecv_money'];
	$formatdata['interestrate_id'] = intval($data['interestrate_id']);
	$formatdata['interestrate_money'] = floatval($data['interestrate_money']);
	$formatdata['learn_id'] = intval($data['learn_id']);
	$formatdata['learn_money'] = floatval($data['learn_money']);
...
function get_user_info($extField,$extWhere="",$Type="ROW")
{
	if($extField=="*"){
		...
		$extField = "*,AES_DECRYPT(real_name_encrypt,'".AES_DECRYPT_KEY."') as real_name,AES_DECRYPT(idno_encrypt,'".AES_DECRYPT_KEY."') as idno,AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."') as money,AES_DECRYPT(cunguan_money_encrypt,'".AES_DECRYPT_KEY."') as cunguan_money,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile";
	}
 ...
 * 发放注册奖励
 */
function send_register_reward($user_id,$date_time="",$pid=0,$get_way){
	if($user_id == 0)
		return false;
	if($date_time==""){
	...
	//红包
	//获取未过期的注册红包
	$red_list = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."red_packet_newconfig WHERE grant_type = 1");
	if($red_list){
		foreach($red_list as $k=>$v){
			require_once APP_ROOT_PATH."system/libs/voucher.php";
			red_packet($v['id'],$user_id);
		}
	}
	//加息券
	//获取未过期的注册加息券
    ...
    //体验金 2017.2.15
    //获取未过期的体验金活动列表
	$ecv_list = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."taste_cash_config WHERE status = 1");
	if($ecv_list){
		foreach($ecv_list as $k=>$v){
			require_once APP_ROOT_PATH."system/libs/voucher.php";
			send_taste($v['id'],$user_id,$get_way);
		}
	}
    /*
     * 邀请送体验金
...
//微信通知
//up BY  20170512 1010
function weixin_tmpl_send_20170512($template_id,$user_id,$weixin_data){
...
/*******累计出借积分领取********/
function checkInvestSign($sign,$user_id,$type){
	$t = time();
	$begin_time = mktime(0,0,0,date("m",$t),date("d",$t),date("Y",$t));
	$end_time = mktime(23,59,59,date("m",$t),date("d",$t),date("Y",$t));
	$sign_data = $GLOBALS['db']->getOne("select id from ".DB_PREFIX."user_score_log where user_id = ".$user_id." and create_time between ".$begin_time." and ".$end_time." and score =".$sign." and type =".$type);
	return $sign_data;
}
/*去掉字符串中的空格*/
 ...
 * @param $user_id 用户id
 * @param $device  设备信息
 * @return mixed  体验金的详细信息
 * @author:zhuxiang
 */
function experience_money($user_id,$device){
	$times = time();
	//体验金总金额
	$root['money_total'] = $GLOBALS['db']->getOne("select SUM(money) from ".DB_PREFIX."taste_cash where user_id=".$user_id);
	$root['money_total'] = $root['money_total']?$root['money_total']:"0.00";
	//体验金可用余额
	$root['can_use_money'] = $GLOBALS['db']->getOne("select SUM(money) from ".DB_PREFIX."taste_cash where user_id=".$user_id." and use_status=0 and end_time>=".$times);
	$root['can_use_money']= $root['can_use_money']? $root['can_use_money']:"0.00";
	//体验经已收收益
	$root['incomed'] = $GLOBALS['db']->getOne("select SUM(interest) from ".DB_PREFIX."taste_cash where user_id=".$user_id." and use_status=1 and get_interest_status=1");
	$root['incomed']= $root['incomed']? $root['incomed']:"0.00";
	//体验金待收收益
	$root['incomeing'] = $GLOBALS['db']->getOne("select SUM(interest) from ".DB_PREFIX."taste_cash where user_id=".$user_id." and use_status=1 and get_interest_status=0");
	$root['incomeing']= $root['incomeing']? $root['incomeing']:"0.00";
	//体验金列表
	$experience_money_list = $GLOBALS['db']->getAll("select tc.* from ".DB_PREFIX."taste_cash tc left join ".DB_PREFIX."taste_cash_config tcc on tc.taste_cash_id = tcc.id where tc.user_id=".$user_id." and tcc.status=1");
	//邀请好友数量
	//$friendnum = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user_referer_log where pid=".$user_id);
	$friendnum = friends_numbers($user_id);
	foreach($experience_money_list as $k=> $v){
		$experience_money_config = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."taste_cash_config where id=".$v['taste_cash_id']." and status=1");
		...
		$root['item'][$k]['money'] = strval($experience_money_config['money']); //体验金金额
		$root['item'][$k]['rate'] = $experience_money_config['rate']; //体验金年化收益
		$root['item'][$k]['taste_id'] = $experience_money_config['id']; //体验金年化收益
		...
		if($v['use_status'] ==0 && $v['get_interest_status']==0){
			...
			if($v['end_time']>$times){
				if($experience_money_config['id']==1){
					...
					if($friendnum['friend_num'] >=3 && $friendnum['taste_status_num']>=3){
						$root['item'][$k]['button'] = '立即使用';
						$root['item'][$k]['button_status'] = '1';
					}else{
						$root['item'][$k]['button_status'] = '5';
						$root['item'][$k]['wap_url'] = WAP_SITE_DOMAIN."/index.php?ctl=taste&act=taste_money_share";
						$root['item'][$k]['button'] = '分享领取';
					}
				}else{
					$root['item'][$k]['button_status'] = '1';
					$root['item'][$k]['touzi_status'] = 3;
					$root['item'][$k]['button'] = '立即使用';
				}
			}else{
				$root['item'][$k]['button_status'] = '4';
				$root['item'][$k]['button'] = '已过期';
				/*$experience_money_log = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."taste_cash_log where user_id =".$user_id." and detail='".$experience_money_config['title']."-过期'");
				$root['haha'] = $experience_money_log;
				if(!$experience_money_log){
					experience_log($user_id,$v['id'],$experience_money_config['id'],-$experience_money_config['money'],$device,$experience_money_config['title'].'-过期');
				}*/
			}
			...
			if($usetime>$limit_time){
				if($usetime > $drawtime){
					$root['item'][$k]['button_status'] = '4';
					$root['item'][$k]['button'] = '已过期';
					/*$experience_money_log = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."taste_cash_log where user_id =".$user_id." and detail='".$experience_money_config['title']."-收益-过期'");
					if(!$experience_money_log){
						//收益过期入库
						experience_log($user_id,$v['id'],$experience_money_config['id'],-$v['interest'],$device,$experience_money_config['title'].'-收益-过期');
					}*/
 ...
 * @param $user_id 用户id
 * @param $device  设备信息
 * @return mixed  体验金的使用详细信息
 * @author:zhuxiang
 */
function experience_money_detail($user_id){
	$weekday = array('星期日','星期一','星期二','星期三','星期四','星期五','星期六');
	$user['id'] =11;
	$detail = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."taste_cash_log where cunguan_tag = 1 and user_id=".$user_id." order by create_time desc");
 ...
 * @param $user_id 用户id
 * @param $device  设备信息
 * @return mixed  体验金的使用状态
 * @author:zhuxiang
 */
function experience_money_use($user_id,$id,$device,$seqNo){

	require_once APP_ROOT_PATH."system/libs/user.php";
	$time = time();
	//体验金的详细信息
	$experience = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."taste_cash where id=".$id." and user_id=".$user_id);
	//邀请好友数量
	//$friendnum = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user_referer_log where pid=".$user_id);
	$friendnum = friends_numbers($user_id);
	//用户是否投过资 不含新手标
	$trade = $GLOBALS['db']->getRow("select dl.*,de.is_new from ".DB_PREFIX."deal_load dl left join ".DB_PREFIX."deal de on dl.deal_id=de.id where de.is_new != 1 and dl.user_id=".$user_id);
	//用户是否投过资 含新手标
	$trade11 = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_load where  user_id=".$user_id);
	if($experience){
		if($experience['taste_cash_id'] == 1){ //注册体验金
			if($experience['use_status'] ==0 && $experience['get_interest_status']==0){
				$res = $GLOBALS['db']->query("update ".DB_PREFIX."taste_cash set use_status=1,use_time=".$time." where id = ".$id);
				if($res){
					experience_log($user_id,$id,$experience['taste_cash_id'],-$experience['money'],$device,"使用-".$experience['disc']);
				...
				$switch = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."switch_conf where switch_id = 1 or switch_id = 6");
				...
				if($trade11){
					$res = $GLOBALS['db']->query("update ".DB_PREFIX."taste_cash set get_interest_status=1 ,get_interest_time=".$time." where id = ".$id);
					if($res){
                        $returnTaste = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."taste_cash_log where user_id=".$user_id." and `change` =".$experience['interest']);
                        if($returnTaste){
                            $root['response_code'] = 0;
							...
							experience_log($user_id,$id,$experience['taste_cash_id'],$experience['interest'],$device,$experience['disc'].'-收益',$seqNo);
						
                            //modify_account(array("money"=>$experience['interest']),$user_id,$experience['disc'].'收益',47);

                            $user_info = get_user_info("*","id = ".$user_id);
                            $money_log_info = array();
                            $money_log_info['memo'] = '注册体验金收益';
                            $money_log_info['brief'] = '注册体验金收益';
                            $money_log_info['money'] = $experience['interest'];
                            ...
                            $money_log_info['type'] = 47;
                            $money_log_info['from_user_id'] = $user_id;
                            $result=$GLOBALS['db']->autoExecute(DB_PREFIX."user_money_log",$money_log_info);
                            if ($GLOBALS['user_info']['id'] == $user_id){
                                $GLOBALS['user_info']['money'] = $user_info['money'];
                            }
                            if($result){
                                $sql = "update ".DB_PREFIX."user set money_encrypt = AES_ENCRYPT(ROUND(ifnull(AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."'),0),2) + ".round($experience['interest'],2).",'".AES_DECRYPT_KEY."') where id =".$user_id;
                                $GLOBALS['db']->query($sql);
                            }

                            $root['response_code'] = 1;
                            $root['success_status'] = "1";
                            $root['show_err'] = ' 恭喜你，收益领取成功请到“我的/可用余额”中查看';
                        }
					}
				}else{
					$root['response_code'] = 1;
					$root['success_status'] = "2";
					$root['show_err'] = '完成任意出借后，即可领取体验金收益，是否前去出借？';
				}
			}else{
				$root['response_code'] = 0;
				$root['show_err'] = '您已经领过收益了,不能重复领取';
				...
				if($friendnum['friend_num']>=3 && $friendnum['taste_status_num']>=3){
					...
					if($res){
                        ...
                        if($returnTaste){
                            ...
                            $user_info = get_user_info("*","id = ".$user_id);
                            $money_log_info = array();
                            $money_log_info['memo'] = '分享体验金收益';
                            $money_log_info['brief'] = '分享体验金收益';
                            $money_log_info['money'] = $experience['interest'];
                            $money_log_info['account_money'] = $user_info['money'];
                            $money_log_info['user_id'] = $user_id;
                            $money_log_info['create_time'] = TIME_UTC;
				...
				if($trade){
					...
					if($res){
                        ...
                        if($returnTaste){
                            ...
                            $user_info = get_user_info("*","id = ".$user_id);
                            $money_log_info = array();
                            $money_log_info['memo'] = '出借体验金收益';
                            $money_log_info['brief'] = '出借体验金收益';
                            $money_log_info['money'] = $experience['interest'];
                            $money_log_info['account_money'] = $user_info['money'];
                            $money_log_info['user_id'] = $user_id;
                            $money_log_info['create_time'] = TIME_UTC;
 ...
 * @author:zhuxiang
 */
function experience_log($user_id,$id,$taste_cash_id,$money,$device,$detail,$seqNo){
	$data['user_id'] = $user_id;
	$data['taste_id'] = $id;
	$data['taste_cash_id'] = $taste_cash_id;
	 ...
	 * @author:zhuxiang
	 */
	function friends_numbers($user_id){

		$friends = $GLOBALS['db']->getAll("select id,user_name from ".DB_PREFIX."user where pid=".$user_id);
		$num = array();
		foreach ($friends as $k=>$v){
			$mobile = $GLOBALS['db']->getRow("select id,idcardpassed_time,AES_DECRYPT(mobile_encrypt,'".AES_DECRYPT_KEY."') as mobile,create_time from ".DB_PREFIX."user where id=".$v['id']." and real_name_encrypt !='' and idno_encrypt !='' ");
            $end_time = $mobile['create_time']+(30*24*3600);
			if($mobile['idcardpassed_time'] < $end_time){
                $taste_status = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."taste_cash where user_id =".$mobile['id']." and taste_cash_id = 1");
				if($taste_status['use_status']==1){
                    $register_taste_status[] = $taste_status;
                }
                $mobile['mobile'] = substr($mobile['mobile'], 0, 3).'**'.substr($mobile['mobile'], -2);
                $mobile['create_time'] = to_date($mobile['create_time'],'m/d H:i');
				$mobile['taste_status'] = $taste_status['use_status'];
				$num[]=$mobile;
			}
		}
		return array('friend_info'=>$num,'friend_num'=>count($num),'taste_status_num'=>count($register_taste_status));
	}
	...
	***/
	function machineInfo(){
		...
		switch ($MachineInfo[0]) {
			case 'iOS':
				$jump['GoBack'] = 'iosJumpToGoBack';
				$jump['RechargeRecord'] = 'iosJumpToRechargeRecord';
				$jump['ToProductList'] = 'iosToProductList';
                $jump['ToRegister'] = 'iosToRegister';
                $jump['index'] = 'iosToHomePage';
                $jump['Godeal'] = 'iosGodeal';
                 $jump['MyPage'] = 'iosToMyPage';
                 $jump['Gohongbao'] = 'iosToHongbao';
				break;
			case 'Android':
				$jump['GoBack'] = 'androidJumpToGoBack';
				$jump['RechargeRecord'] = 'androidJumpToRechargeRecord';
				$jump['ToProductList'] = 'androidToProductList';
                $jump['ToRegister'] = 'androidToRegister';
                $jump['index'] = 'AndroidToHomePage';
                $jump['MyPage'] = 'androidToMyPage';
                $jump['Godeal'] = 'androidGodeal';
                $jump['Gohongbao'] = 'androidhongbao';
				break;
			default:
				$jump['GoBack'] = 'jumpToGoBack';
				$jump['RechargeRecord'] = 'jumpToRechargeRecord';
				$jump['ToProductList'] = 'ToProductList';
                $jump['ToRegister'] = 'ToRegister';
                $jump['index'] = 'ToHomePage';
                $jump['MyPage'] = 'ToMyPage';
                $jump['Godeal'] = 'Godeal';
                $jump['Gohongbao'] = 'Gohongbao';
				break;
		}
		return $jump;
	}
     ...
     * 发送微信模板消息请求
     * @param $url  请求链接
     * @param $data 模板消息格式数据 多维数组
     * @return mixed|string
     * @author:zhuxiang
     */
    function request_curl($url,$data){ // 模拟提交数据函数
    ...
    /**微信模板消息数据组装
     * @param $type 模板消息类型 1,收益到期 2,出借成功 3,出借成功 4,账户绑定
     * @param $openid 用户openID
     * @param $jumpurl 跳转链接
     * @param $datas 模板数据嵌入
     * @return array 组装好的模板参数
     * @author:zhuxiang
     */
    function create_request_data($type,$openid,$jumpurl,$datas){
        ...
        switch ($type){
            /*  {{first.DATA}}
                ...
                {{remark.DATA}}
                到期收益微信模板消息通知--内容示例
                尊敬的用户，您有一项出借项目已到期，本金和收益将存入您的账户余额，请注意查收，资金可用于提现或复投。
                项目名称： 车贷-ds545454
                到期日：2013年1月1日
                本金：20000.00元
                收益金额：500.00元
            ...
            */
            ...
            case 1:
                $data =array('data'=>array(
                    "touser"=>$openid,
                    "template_id"=>"kqihSfUD8_dQCjhRBcqlxgU0PVGAE2BaPUXpgQ9GxDI",
                    "url"=>$jumpurl,
                    "data"=>array(
                        "first"=>array("value"=>$datas['first'],"color"=>"#173177"),
                        "income_amount"=>array("value"=>$datas['income_amount'],"color"=>"#173177"),
                        "income_time"=>array("value"=>$datas['date'],"color"=>"#173177"),
                        /*"productType"=>array("value"=>$datas['productType'],"color"=>"#173177"),
                        "name"=>array("value"=>$datas['name'],"color"=>"#173177"),
                        "PayDate"=>array("value"=>$datas['date'],"color"=>"#173177"),
                        "Principal"=>array("value"=>$datas['principal'],"color"=>"#173177"),
                        "Profit"=>array("value"=>$datas['profit'],"color"=>"#173177"),*/
                        "remark"=>array("value"=>$datas['remark'],"color"=>"#173177")
                    )
                ));
            break;
            /*  {{first.DATA}}
            ...
            case 2:
                $data =array('data'=>array(
                    "touser"=>$openid,
                    "template_id"=>"3WVeajggsGI-jdfm1qY5uLuqXyHUi5_gFHTg6ue-rkI",
                    "url"=>$jumpurl,
                    "data"=>array(
                        "first"=>array("value"=>$datas['first'],"color"=>"#173177"),
                        "keyword1"=>array("value"=>$datas['keyword1'],"color"=>"#173177"),
                        "keyword2"=>array("value"=>$datas['keyword2'],"color"=>"#173177"),
                        "keyword3"=>array("value"=>$datas['keyword3'],"color"=>"#173177"),
                        "keyword4"=>array("value"=>$datas['keyword4'],"color"=>"#173177"),
                        "keyword5"=>array("value"=>$datas['keyword5'],"color"=>"#173177"),
                        "remark"=>array("value"=>$datas['remark'],"color"=>"#173177")
                    )
                ));
                break;
            ...
            case 3:
                $data =array('data'=>array(
                    "touser"=>$openid,
                    "template_id"=>"jenJ5aySW84RdOaBjLizbHlQDN4P_vjZbG5RDFSbOYw",
                    "url"=>$jumpurl,
                    "data"=>array(
                        "first"=>array("value"=>$datas['first'],"color"=>"#173177"),
                        "keyword1"=>array("value"=>$datas['keyword1'],"color"=>"#173177"),
            ...
            case 4:
                /*
                 *  您的{{name1.DATA}}账户已经与{{name2.DATA}}账户绑定在一起。
                    绑定时间：{{time.DATA}}
                    {{remark.DATA}}
                    账户绑定微信消息模板--内容示例
                    您好，恭喜您账户绑定成功！
                    您的玖财通账户已经与微信账户绑定在一起。
                    绑定时间：2013年9月10日
                    您将可以享受如下福利：微信官网免密登录、账户信息及时提醒、用微信快捷登录APP等众多福利。
                 */
                $data =array('data'=>array(
                    "touser"=>$openid,
                    "template_id"=>"DhxtuBDImnUZkwXuwb1QIXimqMorzwjYCOGAWA2pZEM",
                    "url"=>$jumpurl,
                    "data"=>array(
                        "first"=>array("value"=>$datas['first'],"color"=>"#173177"),
                        "keyword1"=>array("value"=>$datas['keyword1'],"color"=>"#173177"),
                        "keyword2"=>array("value"=>$datas['keyword2'],"color"=>"#173177"),
                        /*"name1"=>array("value"=>$datas['name1'],"color"=>"#173177"),
                        "name2"=>array("value"=>$datas['name2'],"color"=>"#173177"),
                        "time"=>array("value"=>$datas['time'],"color"=>"#173177"),*/
                        "remark"=>array("value"=>$datas['remark'],"color"=>"#173177")
                    )
                ));
                break;
