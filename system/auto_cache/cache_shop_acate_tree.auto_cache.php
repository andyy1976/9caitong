...
class cache_shop_acate_tree_auto_cache extends auto_cache{
	public function load($param)
	{
		...
		if($cate_list===false)
		{
			...
			$ids_util = new child("article_cate_cg");
			$ids = $ids_util->getChildIds($pid);
			$ids[] = $pid;
			$cate_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."article_cate_cg where is_effect = 1 and is_delete = 0 and id in (".implode(",",$ids).") order by `sort` desc, id desc");
