...
class loantype_2 implements loantype{
	...
	function deal_repay_money($deal){
		...
		//月还本息
		$return['month_repay_money'] = $deal['borrow_amount'] * $deal['rate']/365/100 * $true_repay_time;
		//实际还多少钱 
	...
	function make_repay_plan($deal){
		...
		for($i=0;$i<$true_repay_time;$i++){
			...
			$load_repay['repay_time'] = $load_repay['repay_time']-86400;	
			...
			$load_repay['user_id'] = $deal['user_id'];
			$load_repay['plan_id'] = $deal['plan_id'];
	...
	function make_user_repay_plan($deal,$idx,$repay_day,$true_time,$repay_id,$load_users,&$total_money){
		...
		foreach($load_users as $k=>$v){
			...
			if($idx+1 == $true_repay_time){
				$item['repay_money'] = $item['money'] + $item['money']*$deal['rate']/365/100*$all_repay_time;
				...
				$item['increase_interest'] = $item['increase_interest'];//平台奖励收益
				$item['interestrate_money'] = $v['interestrate_money'];//加息券奖励收益
			}
		...
		foreach($load_users as $kk=>$vv){
			...
			$repay_data['plan_id'] = $vv['plan_id']; //是否是理财计划
			$repay_data['plan_load_id'] = $vv['plan_load_id']; //是否是理财计划
