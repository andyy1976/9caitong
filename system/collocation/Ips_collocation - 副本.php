...
if (isset($read_modules) && $read_modules == TRUE)
{
	...
	/* 插件作者的官方网站 */
	$module['reg_url'] = 'http://www.jiuchengjr.com';
...
class Ips_collocation implements collocation {
	...
	function __construct(){
		...
		if ($collocation_cfg['is_debug'] == 1){
			$this->post_url="http://180.168.26.114:20010/p2p-deposit/gateway.htm";
			$this->ws_url= "http://p2p.ips.net.cn:6778/CreditWS/Service.asmx?wsdl";
		}else{
			$this->post_url="https://UFunds.ips.com.cn/p2p-deposit/gateway.htm";
	...
	function response($request,$class_act){
		...
		$merBillNo = $request["merBillNo"]; //返回的数据，商户订单号不能重复，前 6 位 以 商 户 号 起 始 ， 例 ：123456xxxxx201522 格式不准 
		$ipsBillNo = $request["ipsBillNo"]; //IPS 订单号 成功时返回
		$ipsDoTime = $request["ipsDoTime"]; //IPS 注册日期  yyyy-MM-dd HH:mm:ss
		$ipsAcctNo = $request["ipsAcctNo"]; //IPS 虚拟账号  成功时返回
		$status = $request["status"]; //注册状态    0:失败;1:成功;2:待审核
