...
class Ips_collocation implements collocation {
	...
	function __construct(){
		...
		if ($collocation_cfg['is_debug'] == 1){
			...
			$this->ws_url= "http://180.168.26.114:20010/p2p-deposit/gateway.htm";
		}else{
	...
	function response($request,$class_act){
		...
		$pMerCode = $request["merchantID"]; //商户存管交易账号
		$pErrCode = $request["resultCode"]; //响应码
		$pErrMsg = $request["resultMsg"];//响应信息
		$p3DesXmlPara = $request["response"];//响应信息JSON 格式的字符串整体报文都会以 3DES 加密处理
		$pSign = $request["sign"]; //签名
