<?php
	 ...
	 * 标的登记 及 流标
	 * @param int $deal_id
	 * @param int $pOperationType 标的操作类型，1：新增，2：结束 “新增”代表新增标的，“结束”代表标的正常还清、不 需要再还款戒者标的流标等情况。标的“结束”后，投资 人投标冻结金额、担保方保证金、借款人保证金均自劢解 冻
	...
	function RegisterSubject($deal_id,$pOperationType,$status, $status_msg, $MerCode,$cert_md5,$post_url){
		...
		if ($pOperationType == 1){
			...
			if ($pOperationType == 1){
				$data['pMerBillNo'] = $newdata['merBillNo'] = $deal_id.'D'.get_gmtime();//'商户订单号 否 商户系统唯一不重复',
				$data['pRegDate'] = $newdata['merDate'] = to_date($deal['start_time'],'Y-m-d');//'登记日期：YYYYMMDD ',
			}else if ($pOperationType == 2){
				$data['pMerBillNo'] = $newdata['merBillNo'] = $deal['mer_bill_no'];//'标的登记时提交的订单单号'
				$data['pRegDate'] = $newdata['merDate'] = to_date($deal['start_time'],'Y-m-d');//'商户日期 否 格式：YYYYMMDD ',
			}
			
			$data['pBidNo'] =  $newdata['projectNo'] = $deal_id.time();//'项目 ID 号 ',
			$newdata['projectName'] = $deal['name'];//项目名称
			$newdata['projectType'] = 1;//项目类型 1：P2P;2：众筹
			$newdata['rateType'] = 1;//利率类型 1：固定;2：浮动
			
			$data['pLendAmt'] =  $newdata['projectAmt'] = str_replace(',', '', number_format($deal['borrow_amount'],2));// '借款金额 
			$data['pGuaranteesAmt'] = str_replace(',', '', number_format($deal['guarantees_amt'],2));// '借款保证金，允许冻结的金额，金额单位，不能为负，允 许为0； 借款保证金  <= 10000.00万 ',
			$data['pTrdLendRate'] =  $newdata['rateVal'] = str_replace(',', '',number_format($deal['rate'],2));//'借款利率 否 金额单位，不能为负，允许为0； 借款利率  < 48%，例如：45.12%传入 45.12 ',
			$newdata['rateMinVal'] = "0.00";//最小利率值 类型为浮动时不可为空，百分数表示
			$newdata['rateMaxVal'] = "0.00";//最大利率值 类型为浮动时不可为空，百分数表示
			...
			$data['pTrdCycleValue'] = $newdata['cycVal'] = $deal['repay_time'];
			
			$pLendPurpose = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."deal_loan_type where id = ".intval($deal['type_id']));
			$data['pLendPurpose'] = $newdata['projectUse'] = $pLendPurpose;// '借款用途',
			...
			$data['pOperationType'] = $pOperationType;// '标的操作类型，1：新增，2：结束 “新增”代表新增标的，“结束”代表标的正常还清、不 需要再还款戒者标的流标等情况。标的“结束”后，投资 人投标冻结金额、担保方保证金、借款人保证金均自劢解 冻。 ',
			$data['pLendFee'] = str_replace(',', '', number_format(floatval($deal['services_fee']) / 100 * $deal['borrow_amount'],2));// '借款人手续费 否 金额单位，不能为负，允许为0 这里是平台向借款人收取的费用 ',
			$data['pAcctType'] = $newdata['finaAccType'] = 1;// '账户类型 否 0#机构（暂未开放） ；1#个人 ',
			$data['pIdentNo'] = $newdata['finaCertNo'] = $user['idno'];// '证件号码 否 真实身份证（个人）/由IPS颁发的商户号 ',
			$data['pRealName'] = $newdata['finaName'] = $user['real_name'];// '姓名 否 真实姓名（中文）',
			$data['pIpsAcctNo'] = $newdata['finaIpsAcctNo'] = $user['ips_acct_no'];// 'IPS账户号 否 账户类型为1时，IPS托管账户号（个人） 账户类型为0时，由IPS颁发的商户号 ',
			$newdata['isExcess'] = 0; //是否超额 0：否 1：是
			$newdata['s2SUrl'] = $pS2SUrl; //后台通知地址
			//print_r($data);exit;
			...
			$data['pMerBillNo'] = $newdata['merBillNo'] = $ipsdata['pMerBillNo'];//'标的登记时提交的订单单号' 
				
			$data['pBidNo'] = $newdata['projectNo'] = $deal_id;//'标的号，商户系统唯一不重复 ',
			$data['pRegDate'] = $newdata['merDate'] = to_date($deal['start_time'],'Y-m-d'); //'商户日期 否 格式：YYYYMMDD ',
			$data['pLendAmt'] =  $newdata['projectAmt'] = str_replace(',', '', number_format($ipsdata['pLendAmt'],2));// '借款金额 否 金额单位，不能为负，不允许为0； 借款金额  <= 10000.00万 关于N(9,2)见4.1补充说明 ',
			$data['pGuaranteesAmt'] = str_replace(',', '', number_format($ipsdata['pGuaranteesAmt'],2));// '借款保证金，允许冻结的金额，金额单位，不能为负，允 许为0； 借款保证金  <= 10000.00万 ',
			$data['pTrdLendRate'] = str_replace(',', '',number_format($ipsdata['pTrdLendRate'],2));//'借款利率 否 金额单位，不能为负，允许为0； 借款利率  < 48%，例如：45.12%传入 45.12 ',
			$data['pTrdCycleType'] = $ipsdata['pTrdCycleType'];	
			$data['pTrdCycleValue'] = $newdata['cycVal'] = $ipsdata['pTrdCycleValue'];// '借款周期值 否 借款周期值 借款周期 <= 5年。 如果借款周期类型为天，则借款周期值<= 1800(360 * 5)；如果借款周期类型为月，则借款周期值<= 60(12 * 5) ',
				
			$data['pLendPurpose'] = $newdata['projectUse'] = $ipsdata['pLendPurpose'];// '借款用途',
			$data['pRepayMode'] = $ipsdata['pRepayMode'];
				
			$data['pOperationType'] = $pOperationType;// '标的操作类型，1：新增，2：结束 “新增”代表新增标的，“结束”代表标的正常还清、不 需要再还款戒者标的流标等情况。标的“结束”后，投资 人投标冻结金额、担保方保证金、借款人保证金均自劢解 冻。 ',
			$data['pLendFee'] =  $ipsdata['pLendFee'];// '借款人手续费 否 金额单位，不能为负，允许为0 这里是平台向借款人收取的费用 ',
			$data['pAcctType'] = $newdata['finaAccType'] =  $ipsdata['pAcctType'];// '账户类型 否 0#机构（暂未开放） ；1#个人 ',
			$data['pIdentNo'] =  $newdata['finaCertNo'] = $ipsdata['pIdentNo'];// '证件号码 否 真实身份证（个人）/由IPS颁发的商户号 ',
			$data['pRealName'] =  $newdata['finaName'] = $ipsdata['pRealName'];// '姓名 否 真实姓名（中文）',
			$data['pIpsAcctNo'] = $newdata['finaIpsAcctNo'] =  $ipsdata['pIpsAcctNo'];// 'IPS账户号 否 账户类型为1时，IPS托管账户号（个人） 账户类型为0时，由IPS颁发的商户号 ',
			$newdata['isExcess'] = 0; //是否超额 0：否 1：是
			$newdata['s2SUrl'] = $pS2SUrl; //后台通知地址	
			$newdata['projectName'] = $deal['name'];//项目名称
			$newdata['projectType'] = 1;//项目类型 1：P2P;2：众筹
...
//print_r($newdata); die;
			$jsonrequest = json_encode($newdata);
			$Crypt3Des=new Crypt3Des();//new 3des class
			$p3DesrequestPara=$Crypt3Des->DESEncrypt($jsonrequest);//3des 加密
			$operationType = "project.regProject";
			$str=$operationType.$MerCode.$p3DesrequestPara.$cert_md5;
			...
			<input type="hidden" name="request" value="'.$p3DesrequestPara.'" />
			</form>
	...
	function RegisterSubjectCallBack($pMerCode,$pErrCode,$pErrMsg,$str3Req){
		...
		if ($GLOBALS['db']->affected_rows()){
			...
			//操作成功
			$data = array();
			$data['projectNo'] = $str3Req["projectNo"]; //
			$data['pIpsBillNo'] = $str3Req["ipsBillNo"]; //IPS 订单号 
			$data['pIpsTime'] = $str3Req["ipsDoTime"]; //'IPS处理时间 否 格式为：yyyyMMddHHmmss ',
