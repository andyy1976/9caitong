...
define("ACCOUNT_NO_VERIFY_ERROR",3); //帐户未激活
	 ...
	 */
	function save_collect_user333($user_data,$mode='INSERT')
	{		
		//开始数据验证
...
//		$user['vip_state'] = 1;
	 ...
	 * @param integer $type  0结存，1充值，2投标成功，3招标成功，4偿还本息，5回收本息，6提前还款，7提前回收，8申请提现，9提现手续费，10借款管理费，11逾期罚息，12逾期管理费，13人工充值，14借款服务费，15出售债权，16购买债权，17债权转让管理费，18开户奖励，19流标还返，20投标管理费，21投标逾期收入，22兑换，23邀请返利，24投标返利，26红包，27代金券 ，28投资奖励，29红包奖励
	...
	function modify_account($data,$user_id,$log_msg='',$type=0)
	{
		...
		if (isset($data['ecv_money'])&&intval($data['ecv_money'])){
			$money_log_info = array();
			$money_log_info['memo'] = $log_msg.',代金券id:'.$data['ecv_id'];
...
/**
	...
	function is_share($user_id){
	    ...
	    $share_rs = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_share_log where user_id = ".$user_id." and share_time between ".$begin_time." and ".$end_time);
	 ...
	 * 查询某会员当日是否领取分享积分
	 */
	function is_receive_share_score($user_id){
	    if($user_id <= 0) return false;
	    $t = time();
	    $begin_time = mktime(0,0,0,date("m",$t),date("d",$t),date("Y",$t));
	    $end_time = mktime(23,59,59,date("m",$t),date("d",$t),date("Y",$t));
	    $share_rs = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_score_log where user_id = ".$user_id." and type=70 and create_time between ".$begin_time." and ".$end_time);
	    if($share_rs) return $share_rs;
	    return false;
	}
	/*
	...
	***/
	function get_referer_list($id,$time_type=0){
		if($time_type == 1){
			$act = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."app_activity where id=".$id);
			$act['create_time'] = strtotime($act['add_time']);
			$act['end_time'] = strtotime($act['end_time']);
			$referer_uid = $GLOBALS['db']->getAll("select uid from ".DB_PREFIX."user_referer_log where pid = ".$GLOBALS['user_info']['id']." and create_time between ".$act["create_time"]." and ".$act['end_time']);
			...
			$referer_uid = $GLOBALS['db']->getAll("select uid from ".DB_PREFIX."user_referer_log where pid = ".$GLOBALS['user_info']['id']);
			foreach ($referer_uid as $k => $v) {
