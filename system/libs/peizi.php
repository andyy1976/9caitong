...
function pay_fee($order_id,$trade_money,$rate_money,$site_money,$allow_send_msg = true){
	...
	if($GLOBALS['db']->affected_rows()){
		...
		//计息金额,用于计算rate_money,site_money的基数,按借款金额计算=peiziorder.borrow_money；如果按实际使用资金计息方式则为=\r\n1、当天新增+	===》利息，佣金\r\n2、当天卖出+	===》利息，佣金\r\n3、当天持有+（不含：当天新增，卖出部分）===》利息
		...
		*/
		...
		//投资推荐人（p_invest_user_id）获得的: 投资人利息收益的n%返利 = rate_money * peiziorder.invite_invest_interest_rate
		...
		//借款推荐人（p_user_id）获得的: 平台佣金收益的n%返利 = site_commission_money * peiziorder.invite_borrow_commission_rate
		...
		if($GLOBALS['db']->affected_rows()){
			...
			/*
			 //投资推荐人（p_invest_user_id）获得的: 投资人佣金收益的n%返利 = invest_commission_money * peiziorder.invite_invest_commission_rate
			...
			//借款推荐人（p_user_id）获得的: 平台利息收益的n%返利 = site_money * peiziorder.invite_borrow_interest_rate
